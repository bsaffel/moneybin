...
@pytest.mark.unit
async def test_register_transform_tools_registers_all_four() -> None:
    """All 4 surviving transform tools register; apply (folded into refresh_run) and restate are excluded."""
    srv = FastMCP("test")
...
async def test_transform_status_pending_appends_action(
    ...
    """When pending=True, actions list includes a refresh_run(steps=['transform']) hint."""
    canned = TransformStatus(
        environment="prod",
        initialized=True,
        last_apply_at=None,
