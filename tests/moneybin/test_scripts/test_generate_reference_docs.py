...
toy_app = typer.Typer(add_completion=False)
db_app = typer.Typer()
toy_app.add_typer(db_app, name="db", help="Database management.")


@toy_app.callback()
def toy(
    home: Path | None = typer.Option(
        None, "--home", envvar="TOY_HOME", help="Data directory."
    ),
) -> None:
    """Toy CLI."""
...
def restore(
    ...
    passphrase: bool = typer.Option(
        False, "--passphrase", help="Derive the key from a passphrase."
    ),
    output: Literal["text", "json"] = typer.Option(
        "text", "-o", "--output", show_default=True, help="Output format."
    ),
) -> None:
    """Restore a backup.
...
@toy_app.command()
def stats(
    limit: int = typer.Option(5, "--limit", min=1, show_default=True),
...
def _toy_root() -> TyperGroup:
    root = get_command(toy_app)
    assert isinstance(root, TyperGroup)
    return root


def test_cli_pages_are_one_per_top_level_command_plus_an_index() -> None:
    pages = render_cli_pages(_toy_root())
...
def test_cli_page_renders_usage_arguments_and_options() -> None:
    page = render_cli_pages(_toy_root())["db.md"]
...
def test_cli_page_renders_numeric_ranges() -> None:
    page = render_cli_pages(_toy_root())["stats.md"]
...
def test_cli_page_fences_indented_help_blocks_and_keeps_lists_as_prose() -> None:
    ...
    is escaped.
    """
    page = render_cli_pages(_toy_root())["preview.md"]
    assert (
