...
def test_dedup_collapsing_two_transfer_legs_retires_the_later_transfer(
    svc: AccountLinksService, db: Database
) -> None:
    """Two accepted transfers, one shared leg after dedup — the later one goes.

    `bridge_transfers` resolves each leg through the dedup mapping
    (`MAX(transaction_id)` per group), so once `ofx_p` and `csv_c` land in one
    component both decisions name the same debit transaction. That is one
    physical transaction acting as the outgoing side of two transfers, which
    double-counts it in every report joining `fct_transactions` to
    `bridge_transfers`. Tier 4 already refuses to *propose* that shape
    (`engine.py` excludes rows in active transfers); this keeps decisions made
    before the merge from arriving at it from the other direction.

    The earliest-decided transfer keeps its component, so the retirement is
...
def test_transfers_on_distinct_components_all_survive(
    svc: AccountLinksService, db: Database
) -> None:
    """The same two transfers with no dedup edge joining them — nothing retires.

    Differs from the fixture above by exactly one property: whether a dedup
    decision links the two debit legs. Without this partner, a retirement that
    keyed on "two accepted transfers on one account" rather than on a shared
    component would pass the test above and quietly delete correct transfers.
    """
    _insert_transfer(
        db,
        match_id="tx_keep00001",
    ...
    statuses = [
        s
        for (s,) in db.execute(
            "SELECT match_status FROM app.match_decisions WHERE match_type = 'transfer'"
        ).fetchall()
    ]
...
def test_a_transfer_whose_own_two_legs_dedup_together_is_retired(
    ...
    `repoint_account` already retires the account-level form of this (the two
    legs collapsing onto one account). This is the transaction-level form, and
    it is equally impossible: `bridge_transfers` would emit a row whose debit
    and credit `transaction_id` are the same.
    """
    _insert_transfer(
        db,
    ...
    assert retired == 1
    status = db.execute(
        "SELECT match_status FROM app.match_decisions WHERE match_id = 'tx_self00001'"
    ).fetchone()
