...
def test_write_categorization_rejects_unknown_categorized_by(
    real_db: Database, fresh_txn: str
) -> None:
    """Unknown ``categorized_by`` raises ``ValueError`` rather than silently failing.

    The SQL CASE generated by ``_priority_case_sql`` has no ELSE branch, so an
    unknown source would resolve to NULL priority — the ON CONFLICT guard
    would silently reject every write and report it as
    ``lower_priority_source``. Validating up front turns a typo or
    refactor-leftover into a loud failure at the call site.
    """
    svc = CategorizationService(real_db)
    with pytest.raises(ValueError, match="Unknown categorized_by"):
        svc.write_categorization(
            transaction_id=fresh_txn,
            category="Food & Dining",
            subcategory=None,
            categorized_by="manual",  # not in _SOURCE_PRIORITY
        )
