...
def test_refresh_rebuilds_transforms_after_writing_rates(
    ...
    assert [stage.step for stage in result.stages].count("transform") == 1
    assert result.stage("transform") == StageOutcome(step="transform", ran=True)
...
def test_refresh_surfaces_failure_while_consuming_written_rates(
    ...
    assert [stage.step for stage in result.stages].count("transform") == 1
    assert result.stage("transform") == StageOutcome(
        step="transform", ran=False, error="SecondPassError"
    )
