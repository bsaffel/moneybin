"""Tests for V039 categorization-decision store creation and backfill."""
...
from moneybin.sql.migrations.V039__create_categorization_decisions import migrate
from tests.moneybin.migration_helpers import run_migration


def test_v039_backfills_three_existing_categories_deterministically(
    db: Database,
) -> None:
    ...
    audit_rows = db.execute(
        """
        SELECT target_id, operation_id, context_json
        FROM app.audit_log
        WHERE action = 'categorization_decision.backfill'
        ORDER BY target_id
        """
    ).fetchall()
    assert audit_rows == sorted([
        (
            categorization_decision_id(row[0]),
            f"op_migration_v039_{categorization_decision_id(row[0])}",
            '{"migration": "V039"}',
        )
        for row in rows
    ])
...
def test_v039_is_idempotent(db: Database) -> None:
    db.execute("DROP TABLE IF EXISTS app.categorization_decisions")
    db.execute(
        """
        INSERT INTO app.transaction_categories (
...
def test_v039_stops_on_orphaned_legacy_accepted_rows(
    db: Database,
