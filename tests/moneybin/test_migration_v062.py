"""Tests for V062 display-currency-target persistence."""
...
from moneybin.sql.migrations.V062__add_profile_display_currency_targets import migrate
...
_MIGRATION_PATH = (
    ...
    / "sql"
    / "migrations"
    / "V062__add_profile_display_currency_targets.py"
)
...
def test_v062_adds_an_empty_default_target_collection(db: Database) -> None:
...
def test_v062_add_column_applies_comment_and_classification_immediately(
    db: Database,
) -> None:
    ...
    and silently skips a column that does not exist yet, so V062 must set
    them itself (Codex CONSIDER on V062 line 32).
    """
    db.execute("ALTER TABLE app.profile_settings DROP COLUMN display_currency_targets")
    migration = Migration.from_file(_MIGRATION_PATH)
...
def test_v062_runs_through_the_migration_runner_transaction(db: Database) -> None:
    """The runner survives V062's required commit/reopen boundary and records it."""
    db.execute("ALTER TABLE app.profile_settings DROP COLUMN display_currency_targets")
    migration = Migration.from_file(_MIGRATION_PATH)
    ...
    assert db.execute(
        "SELECT success FROM app.schema_migrations WHERE version = 62"
    ).fetchone() == (True,)


def test_v062_runner_recovers_a_partial_column_without_a_failure_row(
    db: Database,
) -> None:
...
def test_v044_to_v062_runner_chain_matches_fresh_profile_shape(
...
def test_v062_rerun_and_partial_column_both_restore_the_empty_default(
    db: Database,
) -> None:
