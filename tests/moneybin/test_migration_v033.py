"""Tests for V033: create the investment tables + cost-basis default column.

V033 lays the schema foundation for the investments data model (M1J.1):
...
BEGIN/COMMIT transaction. The fixture reverses the V033 end-state first
...
from moneybin.sql.migrations.V033__add_investment_tables import migrate
...
def _recreate_pre_v033_state(db: Database) -> None:
    """Reverse the V033 end-state: drop the tables and the settings column."""
...
class TestV033Migration:
    """V033 creates the investment tables and settings column, idempotently."""

    def test_creates_tables_and_column(self, db: Database) -> None:
        _recreate_pre_v033_state(db)
