...
V033 touches existing data (``ADD COLUMN`` + backfill ``UPDATE`` + ``SET NOT
...
from moneybin.sql.migrations.V033__add_transaction_categories_source_type import (
    migrate,
)
...
def v032_db(db: Database) -> Database:
    """Database with three existing transaction_categories rows under the pre-V033 shape.
...
@pytest.mark.unit
class TestV033:
    """V033 adds source_type and backfills existing rows to 'internal'."""
