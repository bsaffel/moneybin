"""Tests for V040 import-preview trust-state persistence."""
...
def test_v040_creates_import_preview_trust_state_table(db: Database) -> None:
    db.execute("DROP TABLE IF EXISTS raw.import_preview_snapshots")
    db.execute("DROP TABLE IF EXISTS app.import_previews")
    migration = importlib.import_module(
        "moneybin.sql.migrations.V040__create_import_previews"
    )
...
def test_v040_is_idempotent(db: Database) -> None:
    migration = importlib.import_module(
        "moneybin.sql.migrations.V040__create_import_previews"
    )
