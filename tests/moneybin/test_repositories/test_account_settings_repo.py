...
def _legacy_row(*, account_id: str, archived: bool) -> dict[str, Any]:
    """A full-row capture shaped like a pre-V063 audit image (no archived_at key)."""
    return {
...
def test_undo_of_current_capture_leaves_archived_at_override_untouched(
    ...
    """A post-V063 capture already carries archived_at; the override must not clobber it."""
    repo = AccountSettingsRepo(db)
...
def test_undo_of_legacy_archive_row_preserves_live_archived_at_in_capture(
    ...
    standing in for a V063 backfill or any other post-V063 write -- even
    though the AUDIT ROW being undone is a legacy (pre-V063) capture missing
...
def test_undo_of_undo_of_legacy_archive_row_succeeds(db: Database) -> None:
    ...
    real ``archived_at`` (``2024-03-15``, standing in for a V063 backfill or
    any other post-V063 write) BEFORE the undo below ever runs -- i.e. the
    catalog is migrated, even though the audit row being undone is a legacy
    (pre-V063) capture missing the key entirely. Per ``_archived_at_supported``'s
...
def test_undo_of_undo_of_legacy_first_write_archive_normalizes_archived_at(
    ...
    """Undo-of-undo of a pre-V063 FIRST-write archive must preserve archived_at.

    When archiving was a pre-V063 account's first settings write, its
    ``before_value`` is NULL, so undoing that event takes ``undo_event``'s
    INSERT-shaped path (``before is None`` -> ``_delete_by_pk``), never
    ``_restore_row``. The live row already carries a real, V063-backfilled
    ...
    # archived_at already backfilled by V063.
    ...
    # (before=NULL) -- a pre-V063 capture, so its after-image has no
    # archived_at key.
...
class TestPreV063SchemaToleranceOnAccountSettingsWrite:
    ...
    ``no_auto_upgrade=True`` never gets V063 applied, not just transiently.
    Unlike the legacy-audit-capture tests above (which simulate a pre-V063
    *audit row* on an already-migrated live table), every fixture here drops
    ...
    def test_set_insert_succeeds(self, pre_v063_rw_db: Database) -> None:
        """Cell 2: the INSERT column list must drop archived_at when absent."""
        repo = AccountSettingsRepo(pre_v063_rw_db)
        event = _set(repo, account_id="acct_pre_v063")
        assert event.target_id == "acct_pre_v063"

        row = pre_v063_rw_db.conn.execute(
    ...
    def test_set_on_conflict_update_succeeds(self, pre_v063_rw_db: Database) -> None:
        """Cell 2: the ON CONFLICT DO UPDATE SET list must also drop archived_at."""
        repo = AccountSettingsRepo(pre_v063_rw_db)
        _set(repo, account_id="acct_pre_v063", display_name="Old")
        _set(repo, account_id="acct_pre_v063", display_name="New")
    ...
    def test_delete_before_capture_succeeds(self, pre_v063_rw_db: Database) -> None:
        """Cell 3: _fetch_row's projection must not crash delete()'s before-capture."""
        repo = AccountSettingsRepo(pre_v063_rw_db)
        _set(repo, account_id="acct_pre_v063")

        event = repo.delete("acct_pre_v063", actor="cli")
        assert event is not None
    ...
    def test_undo_of_archive_transition_succeeds(
        ...
        """
        repo = AccountSettingsRepo(pre_v063_rw_db)
        _set(repo, account_id="acct_pre_v063", archived=False)
        archive_event = _set(repo, account_id="acct_pre_v063", archived=True)
    ...
    def test_undo_of_delete_of_archived_account_succeeds(
        ...
        """
        repo = AccountSettingsRepo(pre_v063_rw_db)
        _set(repo, account_id="acct_pre_v063", archived=True)
        delete_event = repo.delete("acct_pre_v063", actor="cli")
    ...
    def test_undo_of_first_write_archive_succeeds(
        ...
        """
        repo = AccountSettingsRepo(pre_v063_rw_db)
        first_write_event = _set(repo, account_id="acct_pre_v063", archived=True)
