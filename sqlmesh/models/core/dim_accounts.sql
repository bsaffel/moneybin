...
SELECT
  ...
  GREATEST(w.loaded_at, COALESCE(s.updated_at, w.loaded_at)) AS updated_at, /* Latest of all per-row input timestamps contributing to this row's current values. Does not advance on idempotent SQLMesh re-applies. See docs/specs/core-updated-at-convention.md. */
  COALESCE(
    s.display_name,
