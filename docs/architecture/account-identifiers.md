...
| PDF statements | Source-native key: a document digest (`pdf_doc_{digest}`) synthesized from the exact document bytes; any account anchor found in the statement is separate match evidence, never part of the key | `raw.tabular_accounts.account_id` ← `src/moneybin/services/pdf_account_identity.py` |
