...
**`--account-id` does not change this value.** Pinning an import states which canonical account the file belongs to; the raw row still records what the *source* calls that account. Writing the canonical id into the source-key column instead would make one column mean two things by import flag, and would let a second pin of the same document fork its key and double-count the statement.
...
- **Re-downloading a statement pinned with `--account-id`** — a bank regenerates a PDF with fresh internal metadata, so the bytes, and therefore the document key, differ even when the statement does not. A pin reuses the key that account already answers to for `pdf` rather than minting a new one, so the re-download lands on the same source key and dedups. Without that reuse the key forks while `transaction_id` does not — it folds the canonical account, which the pin holds still — and staging's `(transaction_id, account_id)` dedup passes both copies into `core.fct_transactions`. When the account holds several document keys — one per statement it has adopted — the pin reuses the first in a stable order, since what dedup needs is only that both imports of one statement agree; a document whose own key is already accepted onto another account keeps that key, so the existing binding conflict is still refused rather than hidden behind the substitution.
