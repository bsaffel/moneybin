...
| PDF statements | Source-native key: a document digest (`pdf_doc_{digest}`) derived from the statement's own account anchor | `raw.tabular_accounts.account_id` ← `src/moneybin/services/pdf_account_identity.py` |
