...
- **Allowed schemas:** `core`, `app`, `reports` only. A table reference outside that set — `raw`, `prep`, `meta`, `seeds` included — is refused before execution, even though those schemas are readable through `db query`/`db shell`/external clients (see [the read surface](#the-read-surface) above).
