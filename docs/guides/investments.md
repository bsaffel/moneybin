...
- **A recorded event has no edit or delete.** `import revert` on the event's batch is the undo — including a wrong `--currency`: revert the batch, then re-record with the correct value.
