...
- **Currency.** Every row carries a `currency_code`, and a report never blends currencies into one figure. A multi-currency profile gets its rows interleaved per currency, best-ranked first within each, so a truncated result still represents every currency. See [One display currency](#one-display-currency).
...
`--display-currency EUR` — `display_currency` on the MCP tool — prices a report into one currency at read time. Nothing converted is ever stored: the original amount and currency stay in every table, and the flag omitted gives the original reading back. Rates come from `moneybin refresh`, which caches the rates your own rows imply into the profile's home currency (`profile set home_currency EUR`); a target with no stored rates falls back, and the report says so instead of guessing:
