...
- **Plaid bank sync** — Connect accounts through Plaid Hosted Link via moneybin-sync (the Plaid integration backend you can self-host, or use the hosted instance of). Cursor-based incremental sync by default; `--force` for full re-fetch. Plaid data lands alongside OFX and CSV in the same canonical tables. Cash and credit-card accounts get full transaction history; investment, loan, mortgage, and HSA accounts get loaded too, and their balances count toward net worth as soon as Plaid reports one. The same `sync pull` also pulls securities, investment transactions, and holdings for investment/brokerage accounts straight into the investment ledger (see [Investments](#investments) below) — no separate command. -> [CLI reference](guides/cli-reference.md)
