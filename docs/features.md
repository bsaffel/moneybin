...
- **`--output json` parity with MCP** — Every read command exposes `--output json` and returns the same `{status, summary, data, actions, error?, next_cursor?}` envelope as the corresponding MCP tool, redacted by the same middleware. Agents driving the shell are first-class. Four commands are deliberate exceptions — the `db query` operator bypass, and the `stats` / `logs` / `migrate status` operations-metadata reads — each named in the CLI reference. -> [CLI reference](guides/cli-reference.md)
