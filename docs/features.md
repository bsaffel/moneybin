...
- **Imported category-text curation** — Category text from a CSV/tabular import is never displayed or passed through as a category; it's only ever an input you match to an existing MoneyBin category or use to mint a new one. `moneybin categories mappings pending` groups the raw imported strings into distinct terms (not one row per transaction) with `did_you_mean` suggestions; `moneybin categories mappings set` maps a term via `--into <category_id>` or `--new <name>`, or marks it as carrying no useful category via `--ignore` (categorizes nothing, never asked about again — works for a shipped Plaid mapping too). -> [CLI reference](guides/cli-reference.md)
...
- **Investment match review** — `moneybin investments matches run` plans whole-event matches across sources; `pending` and `history` inspect durable evidence, alternatives, field choices, and replacement context. Planning is also available through `refresh_run(steps=["investment_match"])`, with inspection through `reviews(kind="investment_matches")`; acceptance and Golden-ledger consolidation remain pending. -> [Investments guide](guides/investments.md)
...
- **`reports.realized_fx`** — Realized foreign-exchange gain/loss per consumed Currency lot, with incomplete unmatched-inventory placeholders left visible.
...
- **Read-only SQL — privacy-safe on both surfaces** — `sql_query` (MCP) and `moneybin sql query` (CLI) run read-only `SELECT`/`WITH`/`DESCRIBE`/`SHOW` against five schemas — `core`, `app`, `reports`, `raw`, and `prep`; `meta` and `seeds` are refused — sharing one enforcement primitive: writes, file-access functions, `PRAGMA`, and `EXPLAIN` are blocked, the schema limit binds catalog statements as well as `SELECT`, and each output column is classified via sqlglot lineage so CRITICAL fields (account/routing numbers) are masked (`****<last4>`). `raw` and `prep` carry no per-column declarations, so 33 columns across 16 tables are declared CRITICAL by hand and every other value is scanned for account and SSN shapes at execution. Neither surface is a way around the masking the typed tools apply — but both reach importer output no typed tool exposes, under the weaker of the two mechanisms. App-state mutations (notes, tags, splits, rules) flow through dedicated tools, not raw SQL. (`moneybin db query`/`shell`/`ui` are raw, unmasked operator access.)
- **MCP install across eight clients** — Claude Desktop, Claude Code, Cursor, Windsurf, VS Code, Gemini CLI, Codex (CLI / IDE), and the ChatGPT desktop app (which has hosted Codex since the two apps merged in July 2026, and shares its config). `moneybin mcp install --client <name>` writes the client config. ChatGPT on the **web/mobile** cannot reach a local stdio server; remote MCP transport is planned on the [roadmap](roadmap.md). -> [MCP clients guide](guides/mcp-clients.md)
