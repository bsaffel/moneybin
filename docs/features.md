...
- **MCP install across eight clients** — Claude Desktop, Claude Code, Cursor, Windsurf, VS Code, Gemini CLI, Codex (CLI / Desktop / IDE). `moneybin mcp install --client <name>` writes the client config. ChatGPT is **not** among them: all its surfaces require a remote MCP server, which arrives with M3D. -> [MCP clients guide](guides/mcp-clients.md)
