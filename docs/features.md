...
- **Plaid bank sync** — Connect accounts through Plaid Hosted Link via a configured `moneybin-sync` broker. Cursor-based incremental sync pulls cash, credit-card, and investment data into the same canonical tables as file imports. -> [CLI reference](guides/cli-reference.md)
...
- **MCP install across eight clients** — Claude Desktop, Claude Code, Cursor, Windsurf, VS Code, Gemini CLI, Codex (CLI / Desktop / IDE), and the ChatGPT desktop app (which hosts Codex and shares its config). `moneybin mcp install --client <name>` writes the client config. ChatGPT on the **web/mobile** cannot reach a local stdio server. -> [MCP clients guide](guides/mcp-clients.md)
