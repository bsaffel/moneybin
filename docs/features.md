...
- **Plaid bank sync** — Connect accounts through Plaid Hosted Link via moneybin-sync (the Plaid integration backend you can self-host, or use the hosted instance of). Cursor-based incremental sync by default; `--force` for full re-fetch. Plaid data lands alongside OFX and CSV in the same canonical tables. Cash and credit-card accounts get full transaction history; investment, loan, mortgage, and HSA accounts get loaded too, and their balances count toward net worth as soon as Plaid reports one. Plaid's Investments product (itemized holdings, cost basis) isn't wired up yet — track those positions by hand via `investments add` in the meantime (see [Investments](#investments) below). -> [CLI reference](guides/cli-reference.md)
...
- **MCP install across seven clients** — Claude Desktop, Claude Code, Cursor, Windsurf, VS Code, Gemini CLI, Codex (CLI / Desktop / IDE all share one config). `moneybin mcp install --client <name>` writes the client config. ChatGPT is **not** among them: all its surfaces require a remote MCP server, which arrives with M3D. -> [MCP clients guide](guides/mcp-clients.md)
