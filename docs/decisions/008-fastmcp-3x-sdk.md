...
> progressive-disclosure motivation cited below — a discovery meta-tool +
...
2. **Architectural fit for sensitivity tiers and progressive disclosure.** [ADR-002](002-privacy-tiers.md) defines `low`/`medium`/`high` tiers enforced by privacy middleware. [ADR-003](003-mcp-primary-interface.md) makes MCP the primary interface; this ADR originally paired it with progressive disclosure through a discovery meta-tool and `tools/list_changed`, implemented by a custom `NamespaceRegistry`. That disclosure design is now retired, but the original v1 limitations remain part of the upgrade rationale: middleware did not compose cleanly across tool calls, sensitivity tiers lived in a parallel decorator system, and `NamespaceRegistry` approximated primitives FastMCP added in 2.x and 3.x.
...
5. **NamespaceRegistry → tag-based visibility transforms.**
   - **Historical, superseded decorator example:** tools were proposed to declare progressive-disclosure intent as `@mcp_tool(domain="categorize")`. The registration layer translated `domain` into `mcp.tool(tags={"categorize"})`; a server-level `Visibility(False, tags={"categorize"})` transform hid the tagged set globally. The visibility design was later retired with the discovery meta-tool; `domain` remains classification metadata, not a registry toggle.
   - The then-planned discovery meta-tool would have wrapped `await enable_components(ctx, tags={domain})`. That portion of the migration was later retired; the current server exposes the fixed standard registry.
...
- **FastMCP supplied a migration path away from the custom registry.** The proposed visibility design was later retired in favor of the fixed standard registry, but the upgrade still removed dependence on the bundled v1 snapshot and preserved grouping metadata without making it a disclosure gate.
