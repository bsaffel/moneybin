...
| `institution_slug` | VARCHAR | Canonical institution slug, the value account matching compares. Same `seeds.institutions` join as `institution_name`, taking `slug` instead of `display_name`. Slugifying the display name does not reproduce it (`U.S. Bank` → `u-s-bank`, not `us_bank`), so a consumer that matches on the name drops candidates. |
