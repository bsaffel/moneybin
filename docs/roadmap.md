...
| **M2O** | Queryable internal schemas | 🚧 | Phase 1 shipped: `reports.*` readable through `sql_query`, each column's `DataClass` taken from its declared `@report(classes=…)` map (ADR-013), so masking matches the typed report tools. Phase 2 designed: `raw`/`prep` via CRITICAL-column declarations plus a content-net floor. [`queryable-internal-schemas.md`](specs/queryable-internal-schemas.md). |
