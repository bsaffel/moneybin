...
| **M1J** | Investments core | 🚧 | **M1J.1 foundation (Pillars A+B) ✅ shipped**: securities catalog, investment-transaction ledger, derived lots/realized-gains/holdings, and the four-method cost-basis engine (FIFO/HIFO/specific-ID/average, Decimal precision) reconciled against a hand-labeled full-tax-year 1099-B fixture; top-level `investments` CLI/MCP group. **M1G.4 child ✅ shipped**: Plaid Investments sync feeds the same ledger automatically (see M1G above). **M1J.7 📐 designed**: review-first, provider-neutral matching treats manual and provider histories as whole economic events, deepens `event_group_id` into the stable Golden event identity, and preserves raw observations, curation, provenance, and transaction-id continuity; broad mixed-history containment stays in place until separately promoted. [`investment-event-matching.md`](specs/investment-event-matching.md). **M1J.2 🗓️ planned**: dividend diff-detection — propose dividends missing from the ledger (held-quantity history × provider dividend data) through a visible confirm; deferred out of the Plaid Investments sync spec. **Pillar C.1 ✅ shipped**: broker-carried close valuation (`core.fct_security_prices`, `core.dim_holdings` market value / unrealized gain / `valuation_status`) with no network code. **Pillar C.2 ✅ shipped**: Tiingo and CoinGecko adapters, the `investments prices pull/set/delete/list/token` CLI, per-date user price marks in `app.security_price_overrides` that outrank every provider close, trade-implied prices from executions, feed keys bound through the existing `app.security_links` review queue, and four `system doctor` price checks — including a staleness check that warns when a position is still valued from a close older than its security type allows. Remaining: Pillar C.3 (`core.fct_holdings_daily`) and Pillar D (net-worth integration). **🔒 M1J closes only when cost basis ties to a real broker 1099-B for a full tax year** — still open; the shipped tie-out is a hand-labeled fixture, not yet a real broker statement. [`investments-data-model.md`](specs/investments-data-model.md). |
