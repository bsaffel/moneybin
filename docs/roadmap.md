...
| **M3K** | CLI / MCP UX standards | 📐 | Interaction patterns, output formatting, prompt/resource conventions. **M3K.1** [`agent-visualization.md`](specs/agent-visualization.md) (draft) specifies chart-ready projections + presentation hints. **M3K.2** [`mcp-tool-surface-scaling.md`](specs/mcp-tool-surface-scaling.md) (draft) proposes one bounded, capability-complete registry of approximately 45 intent-shaped tools, a generic report runner, output schemas, and count/metadata/evaluation gates. Capable hosts may defer schemas from that same registry; generic clients receive it in full. Broader `mcp-ux-standards.md` umbrella still planned. |
