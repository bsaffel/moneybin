...
| **M3 — Productization & Distribution** | Evaluation, packaging, web UI, and the opt-in hosted tier | The full suite is green; the anonymized real-data parity check passes; privacy and security checks pass. | 🚧 |

Three releases cut across those gates: **Public Beta** (installable, guided
CLI setup, sync you host yourself), **Early Access** (feature-complete, adds
the minimal browser surface and MoneyBin-hosted sync), and **General
Availability** (v1, publicized). [Release stages](#release-stages) has the bar
for each.
...
| **M1J** | Investment ledger, tax lots, cost basis, realized gains, Plaid investment ingestion | 🚧 ledger shipped; closes on a real-broker 1099-B tie-out; broker-carried market prices shipped (M1J.3 phase C.1 — positions carry market value and unrealized gain), independent feeds and user price marks shipped (C.2 — Tiingo and CoinGecko); review-first whole-event matching designed (M1J.7); the daily series designed (C.3), net-worth integration (M1J.4), dividend diff-detection (M1J.2), split-source symmetry (M1J.5) and pre-window valuation (M1J.6) remain — [investments-data-model.md](specs/investments-data-model.md), [investment-event-matching.md](specs/investment-event-matching.md), [investments-price-feeds.md](specs/investments-price-feeds.md) |
...
| **M1L–M1P** | Recovery completion, pipeline reconciliation, export bundle, anonymized fixtures | 🚧/✅/📐 per increment — see the [spec index](specs/INDEX.md) |
...
| **M2A–M2B** | Curated reports plus net-worth and balance tracking | 🚧 — M2A, M2B.1, and M2B.2 shipped; M2B.3's unanchored-account guard is next and gates the first public release ([reports-net-worth-sql-surface.md](specs/reports-net-worth-sql-surface.md)) |
...
| **M3B** | Packaging and tester distribution (PyPI, MCP Registry, and `.mcpb` for Public Beta; brew follows) | 🚧 release automation in place; no published package yet |
| **M3C** | Local web UI for review, data quality, accounts, reports | 📐 — [ui-architecture.md](specs/ui-architecture.md) |
| **M3D, M3H, M3J** | Remote MCP with auth (unlocks ChatGPT web and claude.ai connectors), opt-in hosted tier, self-host operations | 🗓️ planned; remote MCP and hosted sync are Early Access |
...
Three releases, in order. Each one's bar is a set of conditions, not a date.

| Release | What it is | Public contracts |
|---|---|---|
| **Public Beta** | Installable test build. Guided CLI setup, local stdio MCP, sync through a server you host with your own Plaid credentials. No announcement. | May still change without a migration path. |
| **Early Access** | Feature-complete. Adds the minimal browser surface (M3A on M3L), MoneyBin-hosted sync, and authenticated remote MCP (M3D). | Freeze here: CLI commands, MCP tools, `core`/`app` schema, on-disk formats. |
| **General Availability** | v1, publicized. Early Access plus the remaining investment, tax and asset, everyday-planning, and extensibility work. | Changes need a migration or deprecation path. |

#### Public Beta bar

Public Beta precedes the milestone gates above. Its bar is deliberately narrower than Ingestion-Complete — it makes MoneyBin *installable* without declaring the engine *done*:

1. **Imports validated** — every shipped import format driven end-to-end against real data.
2. **Net worth correct out of the box** — a fresh install produces an accurate net-worth number across every connected source, or says plainly that it cannot. This is the engine, not the analysis on top of it: a currency with no rate makes the total visibly incomplete rather than silently contributing zero, and so does any eligible account with evidence of holding value — priced holdings, or recorded transaction activity — and no balance observation of any kind, whether that account is a NULL-balance Plaid connection, a tabular import with no balance column, or a manual account with postings and no assertion (M2B.3's guard). Sophisticated analysis waits; a wrong number does not get to ship. (M2B.2 Requirement 7 and M2B.3 — [`reports-net-worth-sql-surface.md`](specs/reports-net-worth-sql-surface.md).)
3. **Self-hosted Plaid round-trip** — link → sync → reconciled balances on a real account, through a sync server the user runs with their own Plaid credentials. Built and validated end-to-end against a Production Plaid account by the author; not yet exercised by a non-author user.
...
The distribution channels split on one line: **does the channel require a vendor human-review workflow?** Self-serve channels that don't — PyPI, the `.mcpb` bundle, the Claude Code plugin, a Homebrew tap, the MCP Registry — are *tester distribution*: they may be prepared and made available to testers ahead of Public Beta. Only first-party directory listings that do require human review (M3O) are held until Public Beta is validated. (Full ladder: [`ai-client-compatibility.md`](specs/ai-client-compatibility.md).)

Investments (M1J) jumped this queue: its foundation (Pillars A+B — the investment-transaction ledger and four-method cost-basis engine) shipped 2026-07-09, ahead of Public Beta. The remaining post-release wave is **investments Pillars C/D (price feeds, net-worth integration) → account subtype detail & Plaid liabilities (M1X) → multi-currency (M1K) → budgets (M2C)**. Those still close the Ingestion-Complete and Analysis-Complete gates and still gate General Availability; they no longer gate Public Beta. **One piece of Pillar D does not wait**: release bar item 2 asks the net-worth number to be right or to admit it isn't, and that guard is cheap while the integration is not. Until holdings feed net worth, an account with priced holdings or recorded transaction activity and no balance observation must make the total visibly incomplete rather than contribute a silent zero (M2B.3). The same line separates M1K from the release bar: the beta needs M2B.2's rate spine over the rates M1K.2 already captures, not the rest of M1K's conversion work.
...
| **M1G** | Plaid sync | 🚧 | Phase 1 (cash + credit) ✅; the link → sync → reconcile round-trip is built and validated against a Production Plaid account by the author. **Plaid-Investments (M1G.4) ✅ shipped**: securities, investment transactions, and dated holdings snapshots ride the same `sync pull` job into the investment ledger via an adopt-or-mint security-identity resolver (review queue on any ambiguity) and an opening-lot bootstrap for pre-window positions; `system doctor` gains nine investment checks. The fee-inclusion convention ships a conservative default pending Plaid Sandbox golden data; split-multiplier derivation also ships a conservative default (every Plaid split is held out of the ledger for review), but **not** pending Sandbox data — no Sandbox payload can carry a split row, so M1J.5 owns the derivation contract instead; staging event groups remain NULL, while M1J.7 owns validated reinvest construction and explicitly defers compound internal-transfer and corporate-action construction. End-to-end integration tests are gated on moneybin-sync implementing the contract. [`sync-plaid-investments.md`](specs/sync-plaid-investments.md). Remaining: validation by a non-author user; SimpleFIN planned. [`sync-plaid.md`](specs/sync-plaid.md). |
...
| **M1I** | Native PDF import | Shipped | Phase 1 seed path shipped (PR #228). Phase 2a deterministic recipe ladder + replay + transactions routing shipped (PR #233): auto-derived recipes persist to `app.pdf_formats` keyed by layout fingerprint and replay for free on the next statement; reconciliation gate (±1¢) routes transaction-shaped PDFs to `raw.tabular_transactions`. Phase 2b shipped: a layout the deterministic rung can't crack escalates to the driving agent (`import_files`/`import_preview` return a bridge payload), and `import_confirm(preview_id=..., bridge_response=...)` re-runs the agent's recipe, reconciles, persists, and loads (MCP-only; gated on the agent caller); a drifted saved recipe auto-`bump_version`s on replay-guard re-derive; a scanned/image-only PDF (no text layer) returns an explicit unsupported outcome. The in-process LLM/vision rung (reading scanned PDFs without a driving agent) is deferred (Out of Scope). [`smart-import-pdf.md`](specs/smart-import-pdf.md). |
| **M1J** | Investments core | 🚧 | **M1J.1 foundation (Pillars A+B) ✅ shipped**: securities catalog, investment-transaction ledger, derived lots/realized-gains/holdings, and the four-method cost-basis engine (FIFO/HIFO/specific-ID/average, Decimal precision) reconciled against a hand-labeled full-tax-year 1099-B fixture; top-level `investments` CLI/MCP group. **M1G.4 child ✅ shipped**: Plaid Investments sync feeds the same ledger automatically (see M1G above) — manual entry and Plaid sync are not yet deduped on one account, so the overlap is **contained rather than tolerated**: `core.dim_holdings` withholds every figure for a position whose account carries two source ledgers (`valuation_status = 'source_overlap'`), `system doctor`'s `investment_source_overlap` check **fails** (exit 1, a release gate) and names the one remedy that clears it — revert the redundant import batch; a disconnect keeps the rows already pulled — and the holdings, lots and gains reads set `summary.degraded_reason`. Investment dedup across sources is M1J.7. **M1J.7 📐 designed**: review-first, source-neutral matching treats manual and aggregator histories as whole economic events, deepens `event_group_id` into the stable Golden event identity, and preserves raw observations, curation, provenance, and transaction-id continuity; the visible-collision guard stays in place until separately promoted. [`investment-event-matching.md`](specs/investment-event-matching.md). **M1J.2 🗓️ planned**: dividend diff-detection — propose dividends missing from the ledger (held-quantity history × aggregator dividend data) through a visible confirm; deferred out of the Plaid Investments sync spec. **Pillar C.1 ✅ shipped**: broker-carried close valuation (`core.fct_security_prices`, `core.dim_holdings` market value / unrealized gain / `valuation_status`) with no network code. **Pillar C.2 ✅ shipped**: Tiingo and CoinGecko adapters, the `investments prices pull/set/delete/list/token` CLI, per-date user price marks in `app.security_price_overrides` that outrank every market-data-vendor close, trade-implied prices from executions, feed keys bound through the existing `app.security_links` review queue, and four `system doctor` price checks — including a staleness check that warns when a position is still valued from a close older than its security type allows. Remaining: Pillar C.3 (`core.fct_holdings_daily`) and Pillar D (net-worth integration). **🔒 M1J closes only when cost basis ties to a real broker 1099-B for a full tax year** — still open; the shipped tie-out is a hand-labeled fixture, not yet a real broker statement. [`investments-data-model.md`](specs/investments-data-model.md). |
| **M1K** | Multi-currency schema wave | 🚧 | Original currency canonical at every grain; conversion staged on top. **M1K.1 capture + integrity ✅ closed**: every source captures the currency it was given (no blind `'USD'`), `core` carries `currency_code` at every monetary grain, a profile-level `home_currency` lives in `app.profile_settings` behind an audited repo, and every report that sums money sub-totals per currency instead of blending — `system doctor`'s `currency_integrity` check fails on unknown-currency rows and warns on mixed ones. Single-currency profiles are unchanged. **M1K.2 🚧**: display conversion via auditable Frankfurter rates (after investments) — the rate layer landed first: `raw.exchange_rates` caching what the provider published, `app.exchange_rate_overrides` behind an audited repo, and a top-level `fx` CLI that resolves one rate, lists a pair's series, and records or removes a correction. A multi-currency demo persona followed — `moneybin demo --persona international` builds five accounts in EUR, GBP, CAD, AED, and USD, one of them outside the rate provider's published set, so the unpriced-pair path is reachable from a shipped fixture. Gathering then moved onto refresh: a best-effort `rates` step closes the cascade, fetching each needed pair's whole span while the pipeline already holds the exclusive writer lock, so a report read prices rows without a network call or a cache write. Read-time conversion came next: `--display-currency` on every report command and `display_currency` on the `reports` tool, defaulting to the profile's home currency, pricing the three reports whose rows are one event on one date and leaving the five that aggregate per currency sub-totalled with a machine-readable reason. `investments holdings` totals a mixed-currency portfolio at each position's own close rate. Requirement 16 stays open: holdings do not contribute to net worth at all yet, which is Pillar D, and the converted daily series additionally needs `core.fct_holdings_daily` (Pillar C.3). **M1K.3 🚧**: Core conversion, Currency-lot, and realized-FX foundation implemented 2026-09-04 by reusing the unchanged investments cost-basis engine; bounded metrics and deterministic $5.00-per-completed-month synthetic ground truth cover it. The per-consumed-lot `core:realized_fx` report is implemented with incomplete coverage visible. Only the deliberate EUR/USD statement tie-out remains, so M1K.3 and MB-111 stay in progress. [`multi-currency.md`](specs/multi-currency.md). |
...
| **M1S** | Cross-source account identity resolution | ✅ | One real account = one canonical, opaque non-PII `account_id` across OFX/CSV/PDF/Plaid; `app.account_links` registry + resolution ladder (auto-adopt on full-number/token, confirm on `institution+last4`). Architecture + M1S.1–.6 ✅. **M1S.7–.9** ✅ (#257/#258): cross-source linking didn't fire in live testing — three capture gaps (last4 never derived into `dim_accounts`; mutable CSV labels keyed hard → duplicates on rename; exporter name used as institution → aggregator bridge dead), closed by capture-into-`dim` + capture contract, CSV bind-first / format-account decoupling, and the exporter↔institution split. All three account-resolving channels (tabular, OFX, PDF) gate before load on an unratified identity, agents included. **Unblocks** cross-source txn dedup and the deferred account merge. [`account-identity-resolution.md`](specs/account-identity-resolution.md). |
...
| **M1Y** | Document–account binding + transaction-identity stability | 📐 | Stops `transaction_id` rotating when a user renames, moves, or re-downloads a file. Splits **document identity** (`source_document_key`, a truncated SHA-256 of the artifact's bytes) from **account identity** (a minted opaque key bound by an explicit human decision), and splits the overloaded `source_transaction_id` into the source's own id (nullable — NULL is now its honest answer) plus MoneyBin's `source_row_key`, selected once as `identity_component`. Today an unregistered tabular file's account key is `slugify(stem) + digest(bytes)`, so next month's export re-keys every row and orphans all `app.*` curation. `source_file` → `source_path` (informational only; already a batch token rather than a path in 3 of 6 channels, and a bare basename in a 4th), and the matcher's physical-import guard moves onto the document key — closing the same-path re-import blind spot. Extends M1S rather than replacing it: a minted key is a `source_native` ref, so `app.account_links` needs no new `ref_kind`. One migration (V052), schema-only and written to be deleted wholesale in a later pre-release reset; it preserves the six `raw.*` tables a re-import cannot rebuild plus the three Plaid investment tables, whose rows are carried across the reshape with `source_document_key` derived in place from the `sync_{job_id}` already in `source_file` — clearing them would permanently change opening positions and cost basis, since the opening-lot bootstrap anchors to the first snapshot and a forced pull can only add the newest. It writes exactly three things beyond schema: a source key per existing manual account, a pre-clear row-count baseline so doctor can report unrecovered Plaid history, and a re-pointing of the `app.account_links` rows whose `source_origin` came from the old `--account-name` fallback, so R8's constant does not strand bindings the user already confirmed. Extends `moneybin system doctor` to all six `app.*` tables hard-coupled to `transaction_id`, since no FK exists there to cascade. [`document-account-binding.md`](specs/document-account-binding.md). |
...
| Address | Area | Status | Notes |
|---|---|---|---|
| **M2A** | Reports recipe library | Shipped | As shipped: 8 registered report routes: six `@report` SQL runners plus two service-backed net-worth routes, backed by seven `reports.*` SQLMesh views. **Superseded by M2B.2**: 10 registered routes, all `@report`-backed over 10 `reports.*` views — no service-backed route remains. |
| **M2B** | Net worth & balance tracking | 🚧 | **M2B.1 ✅ shipped**: daily carry-forward, reconciliation deltas, per-currency aggregation. [`reports-net-worth.md`](specs/reports-net-worth.md). **M2B.2 ✅ shipped**: net worth answerable by a rerunnable query at three grains — one report and one view each, at day, currency x day, and account x day — with home-currency conversion moved into SQL over a window-bounded daily rate spine, balance and rate freshness published as columns, date-scoped archival so closing an account stops rewriting its own history, and both service-backed net-worth reports retired onto `@report` runners. Each report takes the same optional date range, so the snapshot/history split retires with them. Also sets the repo-wide rule that a report's id, view, and CLI command share one name. Closes its own slice of the release bar: an unpriced currency makes the total visibly incomplete (Requirement 7) rather than contributing a silent zero. **M2B.3 📐 designed**: the unanchored-account guard (Requirement 14 of the same spec), delivered apart from M2B.2 so the release gate does not retire when M2B.2 closes — an eligible account with evidence of holding value (priced holdings or recorded transaction activity) and no balance observation at all makes the total NULL with a count beside it, rather than contributing a silent zero, and carries two `system doctor` checks: `net_worth_unanchored_accounts` (`fail`), which enforces this row's release-gating claim, and `net_worth_stale_balance` (`warn`), which flags an aging balance without blocking release. Scoped to the account rather than the date, because the dated position spine it would otherwise need is Pillar C.3. **Still gates the first public release.** [`reports-net-worth-sql-surface.md`](specs/reports-net-worth-sql-surface.md). |
...
| **M2O** | Queryable internal schemas | ✅ | Phase 1: `reports.*` readable through `sql_query`, each column's `DataClass` taken from its declared `@report(classes=…)` map (ADR-013), so masking matches the typed report tools. Phase 2: `raw` and `prep` readable too, behind 33 CRITICAL-column declarations plus a value-shape floor that masks SSN shapes and runs of eight or more digits in every other text or integer value. Saved reports read the same five schemas. [`queryable-internal-schemas.md`](specs/queryable-internal-schemas.md). |
...
M3 layers packaging, distribution, and the web UI on top of the M1 engine and M2 analysis work above — both still in progress — to make MoneyBin delightful and acquirable.

| Address | Area | Status | Notes |
|---|---|---|---|
| **M3A** | Evaluator/testing surface (**pulled forward**) | 🗓️ | `moneybin demo` preset + first-run wizard + a **narrow** Web review console (categorization/import/doctor/lineage), **built on M3L**. Ships early as a *testing/trust* surface so the M1 core is legible — but it's productization, hence M3. The CLI wizard and demo preset are Public Beta items; the web console is Early Access (see [Release stages](#release-stages)). Demo preset shipped: [`demo-preset.md`](specs/demo-preset.md) (✅ `moneybin demo`). |
| **M3B** | Install & packaging | 🗓️ | PyPI Trusted Publishing + Homebrew formula + `.mcpb` bundle. PyPI, the MCP Registry entry, and the `.mcpb` bundle are Public Beta items; Homebrew follows. Packaging ladder + per-client blessed paths designed in [`ai-client-compatibility.md`](specs/ai-client-compatibility.md) (Claude Code plugin/marketplace, MCP Registry publish, install-badge deep links, Antigravity T1 install). At close, README/docs also add earned quality-gate trust signals (doctor, scenario suite, CI/security checks, privacy/redaction, audit/undo) per [`user-facing-doc-polish.md`](specs/user-facing-doc-polish.md) Requirement 19 — linked only when the underlying check exists and is inspectable. |
...
| **M3H** | Hosted launch | 🗓️ | Auth + billing + per-user encrypted DuckDB + GDPR + on-call. **Deployment choice, not the headline.** MoneyBin-hosted sync ships in Early Access; v1 is General Availability. |
...
| **M3K** | CLI / MCP UX standards | 📐 | Interaction patterns, output formatting, prompt/resource conventions. **M3K.1** [`agent-visualization.md`](specs/agent-visualization.md) (draft) specifies chart-ready projections + presentation hints. **M3K.2** [`mcp-tool-surface-scaling.md`](specs/mcp-tool-surface-scaling.md) (in progress) defines one bounded, capability-complete registry of 50 intent-shaped tools, a generic report runner, consumer-driven output-schema admission, and count/metadata/evaluation gates. Capable hosts may defer schemas from that same registry; generic clients receive it in full. **M3K.3** [`cli-output-coherence.md`](specs/cli-output-coherence.md) (✅ shipped) gives the CLI's text branch three named renderers, per-report default column sets with `--wide`, one money formatter with redundant sign encoding, quiet-on-success checks, and message hygiene that keeps MCP tool and service function names out of human output. Broader `mcp-ux-standards.md` umbrella still planned. |
