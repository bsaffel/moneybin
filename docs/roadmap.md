...
| Import confirmation & confidence | 🚧 | One trust step shared by every smart-import channel (tabular, Sheets, PDF): nothing lands unconfirmed on first contact, a confirmed layout replays silently, and a wrong guess is one obvious step to recover (`import_confirm`). Draft spec ([`smart-import-confirmation.md`](specs/smart-import-confirmation.md)). |
