...
| **M2P** | Reports surface: one contract, dynamic + materialized reports | 🚧 | Umbrella. Ask → answer → crystallize into a verifiable report; three-tier parity (built-in / extension / user-created) with a real graduation path; compounds into a BI tool for your money. **M2P.1 foundation ✅ shipped** (one contract, derived-and-CI-verified column classes, `reports.*` boundary cleanup, authoring rule) · **M2P.2** dynamic reports (`app.user_reports`, ask→save→verify — absorbs M2I) · **M2P.3** materialization & distribution (promotion path + `/moneybin-create-report`). [`reports-overview.md`](specs/reports-overview.md). |
