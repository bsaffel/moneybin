...
| **M1G** | Plaid sync | 🚧 | Phase 1 (cash + credit) ✅; the link → sync → reconcile round-trip is built and validated against a Production Plaid account by the author. Remaining: validation by a non-author user; Plaid-Investments (M1G.4) 📐 spec ready — [`sync-plaid-investments.md`](specs/sync-plaid-investments.md); SimpleFIN planned. [`sync-plaid.md`](specs/sync-plaid.md). |
