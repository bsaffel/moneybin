...
| **M1J** | Investments core | 🚧 | Securities, ledger, lots, realized gain/loss, holdings; four cost-basis methods (FIFO/HIFO/specific-ID/average); Decimal precision. Specs reconciled draft→ready 2026-07-04 (Plaid-validated against the OpenAPI spec — sync child needs no core migration); foundation build (M1J.1) in flight. **🔒 closes only when cost basis ties to a real broker 1099-B for a full tax year.** [`investments-data-model.md`](specs/investments-data-model.md). |
