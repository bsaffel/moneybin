...
2. **Net worth correct out of the box** — a fresh install produces an accurate net-worth number across every connected source, or says plainly that it cannot. This is the engine, not the analysis on top of it: an account whose balance cannot be anchored, or a currency with no rate, makes the total visibly incomplete rather than silently contributing zero. Sophisticated analysis waits; a wrong number does not get to ship. (M2B.2 Requirement 7 and M2B.3 — [`reports-net-worth-sql-surface.md`](specs/reports-net-worth-sql-surface.md).)
