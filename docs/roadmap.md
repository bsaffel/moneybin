...
| **M2A–M2B** | Curated reports plus net-worth and balance tracking | ✅ shipped — M2A, M2B.1, M2B.2, and M2B.3's unanchored-account guard ([reports-net-worth-sql-surface.md](specs/reports-net-worth-sql-surface.md)) |
...
| **M2B** | Net worth & balance tracking | ✅ | **M2B.1 ✅ shipped**: daily carry-forward, reconciliation deltas, per-currency aggregation. [`reports-net-worth.md`](specs/reports-net-worth.md). **M2B.2 ✅ shipped**: net worth answerable by a rerunnable query at three grains — one report and one view each, at day, currency x day, and account x day — with home-currency conversion moved into SQL over a window-bounded daily rate spine, balance and rate freshness published as columns, date-scoped archival so closing an account stops rewriting its own history, and both service-backed net-worth reports retired onto `@report` runners. Each report takes the same optional date range, so the snapshot/history split retires with them. Also sets the repo-wide rule that a report's id, view, and CLI command share one name. Closes its own slice of the release bar: an unpriced currency makes the total visibly incomplete (Requirement 7) rather than contributing a silent zero. **M2B.3 ✅ shipped**: the unanchored-account guard (Requirement 14 of the same spec), delivered apart from M2B.2 so the release gate does not retire when M2B.2 closes — an eligible account with evidence of holding value (priced holdings or recorded transaction activity) and no balance observation at all makes the total NULL with a count beside it, rather than contributing a silent zero, and carries two `system doctor` checks: `net_worth_unanchored_accounts` (`fail`), which enforces this row's release-gating claim, and `net_worth_stale_balance` (`warn`), which flags an aging balance without blocking release. Scoped to the account rather than the date, because the dated position spine it would otherwise need is Pillar C.3. [`reports-net-worth-sql-surface.md`](specs/reports-net-worth-sql-surface.md). |
