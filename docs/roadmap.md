...
2. **Net worth correct out of the box** — a fresh install produces an accurate net-worth number across every connected source, or says plainly that it cannot. This is the engine, not the analysis on top of it: a currency with no rate makes the total visibly incomplete rather than silently contributing zero, and so does an investment account holding priced value with no balance observation of any kind (M2B.3's guard). A connected account whose provider reports no balance at all — a NULL Plaid `current_balance`, a tabular import with no balance column — is a known gap outside that guard's scope: `core.fct_balances` drops the row rather than anchoring it at zero, and nothing yet flags the resulting silent absence. Sophisticated analysis waits; a wrong number does not get to ship. (M2B.2 Requirement 7 and M2B.3 — [`reports-net-worth-sql-surface.md`](specs/reports-net-worth-sql-surface.md).)
