...
| **M1J** | Investment ledger, tax lots, cost basis, realized gains, Plaid investment ingestion | 🚧 ledger shipped; closes on a real-broker 1099-B tie-out; broker-carried market prices shipped (M1J.3 phase C.1 — positions carry market value and unrealized gain), external feeds and the daily series designed (C.2, C.3), net-worth integration (M1J.4), dividend diff-detection (M1J.2), split-source symmetry (M1J.5) and pre-window valuation (M1J.6) remain — [investments-data-model.md](specs/investments-data-model.md), [investments-price-feeds.md](specs/investments-price-feeds.md) |
