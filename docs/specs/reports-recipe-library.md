...
The current catalog has since grown: ten SQLMesh report views back nine report
...
slice. `reports.net_worth_accounts` and `reports.net_worth_currencies`,
specified by
[`reports-net-worth-sql-surface.md`](reports-net-worth-sql-surface.md), have no
registered report route yet, so every view is accounted for.
