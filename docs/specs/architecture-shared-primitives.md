...
7. **No PII in logs.** The permitted list is [`privacy-data-protection.md`](privacy-data-protection.md) §"What CAN appear" — record counts, entity ids, masked identifiers, category labels and institution names, status codes and operation names, and file paths (never file contents). Nothing outside it, and no shorter restatement of it. `SanitizedLogFormatter` is a runtime safety net, not a license to be careless. See `.claude/rules/security.md` "PII in Logs and Errors".
...
10. **`app.*` mutation routing.** All mutations of `app.*` tables MUST emit a paired `app.audit_log` row via `audit_service.record_audit_event()` inside the same DuckDB transaction, except for: (a) `app.audit_log` itself, (b) `app.metrics` (observability data, not user state), (c) seed-loaded configuration tables written only at install/migration time (currently `app.seed_source_priority`), and (d) migration-system tables (`app.schema_migrations`, `app.versions`). Direct `INSERT`/`UPDATE`/`DELETE` against `app.*` from outside `audit_service.py` or `*_repo.py` modules is a contract violation. The doctor MUST verify routing via per-table invariants. Established by [`app-integrity-invariant.md`](app-integrity-invariant.md); enforced incrementally as each protected `app.*` table migrates to its `*Repo`, with the lint rule landing last.

## Data Layer

MoneyBin uses eight schemas under this spec — seven that exist today plus `reports`, introduced as a [cascading edit](#cascading-edits). Each has a single owner, a single mutator, and a fixed set of allowed prefixes. Consumers read from `core` and `reports` for analysis; everything else is implementation detail, with one inspection exception recorded in Layer Rule 2 — the agent-safe SQL surface also reads `raw` and `prep`.

| Schema | Materialization | Mutated by | Read by | Allowed prefixes | Purpose |
|---|---|---|---|---|---|
| `raw` | Tables | Python extractors (`src/moneybin/extractors/`) and the import services that drive them; MCP write tools | SQLMesh staging models; agent-safe SQL (inspection, Layer Rule 2) | source-named (`tabular_*`, `ofx_*`, `plaid_*`, `manual_transactions`); plus `<pkg>_*` for analysis-package contributions per [extension-contracts.md](extension-contracts.md) | Untouched data from each source. Re-importable from the original file. |
...
| `core` | Tables (canonical) and Views (derived-grain) | SQLMesh transforms | All consumers (services, MCP, CLI, reports) | `fct_<entity>` (event grain or alternate event grain), `dim_<entity>` (slowly-changing entity), `bridge_<entity>` (M:N relationship), `alias_<entity>_ids` (one current-id resolver with an explicit terminal-retired outcome, derived from canonical history); plus `<pkg>_*` for analysis-package contributions per [extension-contracts.md](extension-contracts.md) | Canonical, deduplicated, multi-source. **One canonical table per real-world entity at its primary grain.** Alternate-grain facts use `fct_*` with a disambiguating name (`fct_transactions` header grain, `fct_transaction_lines` line grain). |
...
| `reports` | Views (typically) | SQLMesh transforms | CLI `reports *` commands; MCP `reports(report_id=..., parameters=...)`; future HTTP `/reports/*` | `<entity>` matching the CLI report name (`networth`, `spending_trend`, `budget`, future `portfolio`, `cash_flow`); plus `<pkg>_*` for analysis-package contributions per [extension-contracts.md](extension-contracts.md) | Curated presentation models, one per report surface. **Read-only by design.** Symmetric with the CLI/MCP `reports` namespace per `moneybin-cli.md` v2. |
...
5. **Multi-source consolidation happens in `core`.** Core models normally
   `UNION ALL` every staging source with a `source_type` column. When an owning
   domain has reviewed cross-source identity state, Core instead projects
   through that App mapping so corresponding observations produce one canonical
   row rather than one row per source. Staging remains the source-normalization
   boundary in either case. Per `.claude/rules/database.md` "Column Name
   Consistency Across Layers"; investment-event membership is the first mapped
   projection under this exception.
6. **Sign convention:** negative = expense, positive = income. `DECIMAL(18,2)` for money amounts, `DECIMAL(18,8)` for quantities and unit prices, `DATE` for dates. Per `.claude/rules/database.md` "Anti-Patterns". Domain-driven precision deviations are allowed when the owning spec documents the rationale — the investment tables use `DECIMAL(28,10)` for quantities and unit prices (crypto fractional-unit precision; see [`investments-data-model.md`](investments-data-model.md) "Precision convention").
...
- **Reads scale; writes serialize.** Any number of read-only connections coexist across processes when no write is active; a read-only open that lands during an active write retries on the same backoff (`get_database(read_only=True)` shares the write-mode retry loop) and only fails if the write exceeds `max_wait`. Write connections are exclusive — a second writer retries until the first releases the per-operation lock and then succeeds, or raises `DatabaseLockError` once `max_wait` is exhausted (e.g., a `transform apply` that runs longer than the retry window).
...
- **Cross-process contention is supported, not prevented.** Two processes opening write connections against the same `.duckdb` file serialize via the retry; readers never block each other. Higher-frequency multi-writer scenarios (the IPC socket-server upgrade path named in ADR-010) are deferred until a real trigger appears — see [§(a)](#a-writer-coordination--settled-by-adr-010).
...
MoneyBin operates one **50-tool standard registry**. Generic clients receive that
...
- **`--home` or `MONEYBIN_HOME` overrides the base directory.** The flag is implemented by exporting the env var before config loads, so it wins when both are given. Otherwise the resolution order is: `MONEYBIN_ENVIRONMENT=development` → `<repo-root>/.moneybin` inside a MoneyBin checkout (or `<cwd>/.moneybin` outside one); repo checkout detection (`.git` + `pyproject.toml` name=moneybin) → `<cwd>/.moneybin`; default → `~/.moneybin/`. Both checkout branches map a linked git worktree to its main checkout, so one repository has one data home.
- **`.env.<profile>` overrides `.env`.** Custom dotenv resolution in `settings_customise_sources` looks up `~/.moneybin/.env.<profile>` first, then `~/.moneybin/.env`. `MONEYBIN_HOME` cannot be set there — the file lives inside the directory it would name — and a startup guard refuses it rather than silently resolving elsewhere.
