...
|---|---|---|---|---|---|
| `raw` | Tables | Python loaders (`src/moneybin/loaders/`); managed-write MCP tools | SQLMesh staging models | source-named (`tabular_*`, `ofx_*`, `plaid_*`, `manual_transactions`) | Untouched data from each source. Re-importable from the original file. |
