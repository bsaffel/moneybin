...
- **Power-user path is `sql_schema(table='<schema>.*')` or `sql_query(query="...")` against the catalog**, not another resource. The listing projection was added because the two sets — what `sql_query` may read (`ALLOWED_QUERY_SCHEMAS`) and what the curated doc describes (the `audience="interface"` `TableRef`s, 35 of 93, plus any live `raw.gsheet_<alias>` / `raw.pdf_<alias>` seed view) — are different, and nothing on the surface said so. The listing's `curated` flag is therefore read off the doc itself, not off `INTERFACE_TABLES`: the seed views are curated at runtime and can never be `TableRef`s, so the static set would contradict the per-table call for exactly the relations a connected user has. An agent that asked `sql_schema` for a queryable-but-uncurated table got `sql_unknown_table`, which is a false negative: it steers toward fixing the name rather than toward `DESCRIBE`. The listing plus the `sql_table_not_curated` code close that.
