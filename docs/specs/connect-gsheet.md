...
| PKCE with an embedded public client ID **and** client secret | Google's Desktop clients require `client_secret` in the code→token exchange even under PKCE — an empty value fails the exchange *after* a successful consent. RFC 8252 §8.5: a credential shipped to every user is not confidential; PKCE and the loopback redirect carry the security. Only Android/iOS/Chrome clients are exempt, because they bind to a signing certificate — desktop has no equivalent attestation. Both halves ship, so a bare install authorizes with no setup; a user who wants their own quota or trust boundary overrides both — see "Embedded credential: what it costs". |
...
6. **Initial pull.** After save, trigger a normal direct pull (same loader path as subsequent pulls). It loads raw data only; run the refresh cascade separately to rebuild derived state.
...
30. **Connection-health projection.** `gsheet(view="status")` returns `data.kind="status"` with `data.connections[]`. Drift is surfaced through canonical top-level `actions` hints for inspection and re-detection; the live payload does not promise aggregate connection or drift counts.
...
34. **MCP tools** mirror CLI capabilities through four workflow boundaries:
    ...
    - `gsheet_disconnect(connection_id=..., state="disconnected")` is reversible; `gsheet_disconnect(connection_id=..., state="absent", ...)` is a confirmed permanent purge.
    - `gsheet_pull(connection_id=None)` pulls one connection or every healthy connection.
    - `gsheet(view="connections")` reads the collection. `gsheet(view="status", connection_id=None)` reads connection health and may narrow to one connection.
...
| `raw.tabular_accounts` | Accounts inferred from multi-account sheets land here as today, with `source_type='gsheet'`. One row per distinct account the sheet names, keyed on first sight by `label_account_key(label)` — the tabular path's derivation for a named account, which unlike bare `slugify` survives a label in a non-Latin script. The key is then remembered against its label and looked up before it is re-derived, because `transaction_id` folds it: recomputing it from an edited label would soft-delete and re-insert every row the account owns. One label leaving as one arrives is adopted as a rename, so the account re-labels rather than minting a twin; an ambiguous set mints and lets the review queue propose the merge. Each is resolved through `AccountResolver`, so an account already known from another source is proposed for merge in the account-link review queue instead of becoming a silent duplicate. |
...
| `src/moneybin/mcp/tools/gsheet.py` | Expose connection and status projections through `gsheet` |
...
accepted industry position for installed apps, not a MoneyBin shortcut.
[RFC 8252 §8.5](https://datatracker.ietf.org/doc/html/rfc8252#section-8.5):
"Secrets that are statically included as part of an app distributed to multiple
users should not be treated as confidential secrets, as one user may inspect
their copy and learn the shared secret" — and it is "NOT RECOMMENDED for
authorization servers to require client authentication of public native apps
clients using a shared secret." Google agrees about its own flow: "it is
...
which is also where the Android/iOS/Chrome exemption is stated — `client_secret`
is "not applicable" only to those, leaving Desktop clients requiring it. Two
costs come with it, and the second is the one that actually bites.

**Current state: both halves ship.** Google's Desktop clients require the pair,
so shipping the ID alone left `authorize()` refusing before it opened a browser
and forced every user to register their own Desktop client — the 15 minutes of
Cloud Console setup this connector chose OAuth to avoid. The pair now ships and
a bare install authorizes with no setup; the costs below are what that buys and
...
pattern, now tells users that "the shared client_id is being retired and will
stop working during 2026" and that creating their own is "required rather than
merely recommended" ([rclone Google Drive docs](https://rclone.org/drive/)).

**Consequence for us.** The embedded credential is a scaling cliff, not a
...
### Gsheet health projection

Connection health is queried through `gsheet(view="status")`, optionally with
`connection_id=...`; it is not a `system_status` block. The runtime envelope uses
canonical top-level `actions` strings for drift inspection and re-detection hints.
The current registry advertises no output schema, so this spec does not freeze an
...
4. `gsheet(view="status")` returns connection health in both empty and needs-attention cases.
5. `refresh_run` default steps include `"gsheet"`; non-complete pulls emit a warning, and callers query per-connection health with `gsheet(view="status")`.
