...
| `import_preview(file_path=..., mapping=...)` | Shape 3 (staged preview) | Persists encrypted preview state while returning the proposed mapping, `Confidence`, samples, and unmapped columns. `mapping` is an optional tabular-only partial-merge override (Requirement 8); a validated override reports `confidence="high"` unless a structural red flag pins the tier to `low`. Its live annotation is `readOnlyHint=false` and `idempotentHint=false`. For PDF, it also emits the bridge payload (IR / page image + extraction request). |
