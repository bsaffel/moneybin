...
| [Database Writer Coordination](database-writer-coordination.md) | Feature | draft | Implement ADR-010: replace long-lived read-write singleton with short-lived purpose-declared connections; `read_only` flag on `Database`; `get_database()` retry; multi-process E2E tests |
