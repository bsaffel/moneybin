...
| [Categorize Bulk](categorize-bulk.md) | Feature | implemented | `moneybin transactions categorize commit` CLI parity for `transactions_categorize_commit` MCP tool (both renamed from `bulk_apply` / `bulk` in PR #171); shared `CategorizationItem` Pydantic model with per-item validation; `RecordingContext` drops per-item duplicate DB lookups |
...
## Standalone
| [Account Management](account-management.md) | Feature | implemented | Owns the `accounts` entity namespace: list/get/set/resolve, reversible account merging via bridge model, per-account settings (`app.account_settings`), display preferences. CLI per `moneybin-cli.md` v2: top-level `accounts` (entity ops; balance lives nested under `accounts balance` per `net-worth.md`). The unified `accounts set` is the partial-update entry point for every settings field, including the behavioral flags (display name, include/exclude, archive/unarchive) that were previously separate verbs. Bundles with `net-worth.md`. |
| [Net Worth & Balance Tracking](net-worth.md) | Feature | implemented | Authoritative balance tracking per account, daily carry-forward interpolation, reconciliation deltas, `agg_net_worth` aggregation; cash-only v1. CLI per `moneybin-cli.md` v2: `accounts balance` (per-account workflow), `reports networth` (cross-domain rollup). Bundles with `account-management.md` (shared `accounts` namespace + `app.account_settings`). |
