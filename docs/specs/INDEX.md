...
| [Core `updated_at` Convention](core-updated-at-convention.md) | Architecture | in-progress | Row-level freshness contract for `core.*` models (`updated_at` = MAX of per-row inputs; NULL for pure-seed rows) and model-level freshness via `meta.model_freshness` (wraps SQLMesh `_snapshots`). Adds `updated_at` to `app.user_categories`, `app.user_merchants`, `app.category_overrides`, `app.merchant_overrides`; replaces misleading `CURRENT_TIMESTAMP AS updated_at` on `dim_accounts`. |
