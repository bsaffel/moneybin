...
| Spec | Type | Status | Summary |
|---|---|---|---|
| [Shared Primitives](architecture-shared-primitives.md) | Architecture | implemented | Reference doc for the 12 primitives that crystallized through M0–M1 — `Database`, `SecretStore`, service-layer contract, `TableRef`, `ResponseEnvelope`, `@mcp_tool`/privacy middleware, `@tracked`/`track_duration`, `SanitizedLogFormatter`, `ingest_dataframe`, `MoneyBinSettings`, SQLMesh layer conventions, scenario fixture format. Names the `app.*` user-state layer and the new `reports.*` presentation layer; cited by every M2+ spec. |
| [Extension Contracts](extension-contracts.md) | Architecture | in-progress | Contributor-facing surface for the three extension types — Reports, Analysis Packages, Providers — with distinct trust postures (open ecosystem for reports/packages with capability-declared constraints; in-tree only for providers). Pluggable via setuptools entry points; framework-validated prefix discipline; Quality Scale (bronze → platinum) cross-cutting all three. 1.0 reference packages: `assets` + `us_tax`; all in-tree extensions ship at Platinum. Four `/moneybin-*` guided-contribution skills + `moneybin extension validate` shipped with the Claude Code plugin. Pre-launch deliverable; locks at M3E alongside MCP/CLI surfaces. |
...
| [Transaction Curation](transaction-curation.md) | Feature | implemented | User-state layer on transactions: manual entry (CLI single, MCP bulk), multi-note threads, multi-tag, split-via-annotation, import-batch labels, unified `app.audit_log` (subsumes `app.ai_audit_log`). New `app.*` curation tables; `core.fct_transactions` extended with LIST/STRUCT presentation; new `core.fct_transaction_lines` view. Establishes CLI-imperative / MCP-declarative-set pattern. M2A entry spec. |
| `web-ui-prototype.md` | Feature | planned | M2C narrow Web UI surface (FastAPI + React): AI-categorization-proposal review queue + transactions list. Treated as the first iteration of the M3D Web UI, not throwaway. Stress-tests the writer-coordination contract from `architecture-shared-primitives.md` under real concurrent reads/writes (sync + MCP + UI) before M3A and M3D depend on it. Scoped narrow: no accounts management, no balance reconciliation, no dashboards (those are M3D scope). |
