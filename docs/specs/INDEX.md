...
| [Transaction Curation](transaction-curation.md) | Feature | draft | User-state layer on transactions: manual entry (CLI single, MCP bulk), multi-note threads, multi-tag, split-via-annotation, import-batch labels, unified `app.audit_log` (subsumes `app.ai_audit_log`). New `app.*` curation tables; `core.fct_transactions` extended with LIST/STRUCT presentation; new `core.vw_transaction_lines` view. Establishes CLI-imperative / MCP-declarative-set pattern. M2A entry spec. |
| [Web UI Prototype](web-ui-prototype.md) | Feature | draft | M2C narrow Web UI surface (FastAPI + React): AI-categorization-proposal review queue + transactions list. Treated as the first iteration of the M3D Web UI, not throwaway. Stress-tests the writer-coordination contract from `architecture-shared-primitives.md` under real concurrent reads/writes (sync + MCP + UI) before M3A and M3D depend on it. Scoped narrow: no accounts management, no balance reconciliation, no dashboards (those are M3D scope). |
