...
| [Transform Handoff](smart-import-transform.md) | Feature | implemented | Ships the five `transform_*` MCP tools as planned in `mcp-tool-surface.md` §1530; renames `import_file` → `import_files` (batch-shaped, auto-applies transforms at end of list); adds `system_status.transforms` block (pending/last_apply_at) so staleness is observable without a separate call. Closes the gap surfaced by the 2026-05-15 silent-stale-`dim_accounts` finding. |
