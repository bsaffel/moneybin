...
| Spec | Type | Status | Summary |
|---|---|---|---|
| [Shared Primitives](architecture-shared-primitives.md) | Architecture | ready | Reference doc for the 12 primitives that crystallized through M0–M1 — `Database`, `SecretStore`, service-layer contract, `TableRef`, `ResponseEnvelope`, `@mcp_tool`/privacy middleware, `@tracked`/`track_duration`, `SanitizedLogFormatter`, `ingest_dataframe`, `MoneyBinSettings`, SQLMesh layer conventions, scenario fixture format. Names the `app.*` user-state layer and the new `reports.*` presentation layer; cited by every M2+ spec. |
