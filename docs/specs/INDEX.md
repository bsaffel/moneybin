...
## Standalone
| [Account Management](account-management.md) | Feature | implemented | Owns the `accounts` entity namespace: list/show/rename/archive/include, reversible account merging via bridge model, per-account settings (`app.account_settings`), display preferences. CLI per `moneybin-cli.md` v2 (extends with archive/merge/unmerge): top-level `accounts` (entity ops; balance lives nested under `accounts balance` per `net-worth.md`). Bundles with `net-worth.md`. |
