...
| [Confirmation & Confidence](smart-import-confirmation.md) | Feature | draft | Cross-channel confirm-the-columns trust layer: one unified confidence contract (normalized score + tier) and one `import_preview`→`import_confirm` flow shared by tabular, gsheet, and PDF. First-encounter always confirms; confirmed layouts reuse silently; tiered agent autonomy (calibration-gated) paired with visible revert/re-map recovery. Owns the `import_confirm` signature + `resolve_or_confirm` primitive that `smart-import-pdf.md` references. |
