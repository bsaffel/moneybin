...
| [Cold Start](categorization-cold-start.md) | Feature | implemented | First-run categorization: per-region seed merchants (US/CA/global, ~2100 entries), `app.merchants` view split (seeds + user_merchants + overrides), LLM-assist workflow with type-enforced PII redaction, CLI export/apply bridge for agent + manual modes; renames `transactions_categorize_bulk_apply` → `_apply` and `auto_confirm` → `auto_accept` |
