...
> - **`match_decisions` coverage watermark is `COALESCE(reversed_at, decided_at)`** (no single `updated_at` column; `reverse` bumps `reversed_at`, `insert`/`update_status` bump `decided_at`). Using the COALESCE means a raw bypass *reversal* is flagged too, not just a bypass insert — the coverage helper gained an `updated_expr` parameter (same generalize-the-helper precedent as `updated_col`/`pk_expr`) to carry the expression.
