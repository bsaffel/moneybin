...
The 49-tool registry uses coarse domain operations with typed projections and
...
- Reserved display name: `settings_update(A, display_name="Unnamed account")` raises before the DB write, as does any case variant; a row already carrying the literal still loads through `_load_settings`.
