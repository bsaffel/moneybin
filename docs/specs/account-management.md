...
4. **Archive and include_in_net_worth are independent; archiving stamps a date.** Setting `archived = TRUE` stamps `archived_at` with today's date the first time the account transitions to archived (a later call while it is already archived leaves the date untouched); `archived = FALSE` clears `archived_at` back to NULL. Neither transition touches `include_in_net_worth` — an archive cascade shipped originally and was retired by [`reports-net-worth-sql-surface.md`](reports-net-worth-sql-surface.md) §Prerequisites, because archiving is a present-tense action while net worth is a stock measure whose history should stay immutable; `archived_at` is recorded on that transition so a future report can exclude the account only for dates after it, instead of retroactively — no report reads it that way yet; today `archived` alone still excludes the account from net worth entirely via the existing blanket filter.
...
CREATE TABLE IF NOT EXISTS app.account_settings (
    ...
    archived_at          DATE                                      -- The date the account stopped being part of the position; NULL while active (added by reports-net-worth-sql-surface.md §Prerequisites). Appended last, not beside `archived`, because DuckDB's ALTER TABLE ADD COLUMN always appends
);
```
...
- **Archive and include are independent.** `--archive` flips `archived = TRUE` and stamps `archived_at`; `--unarchive` flips `archived = FALSE` and clears `archived_at`. `--archive` alone already excludes the account from net worth entirely, history included, via the existing blanket filter; `archived_at` is recorded for a future date-scoped release, not applied yet. `--include`/`--exclude` toggle net-worth inclusion independently of archived status — pass `--exclude` to exclude an account that stays active and listed.
...
The 50-tool registry uses coarse domain operations with typed projections and
...
- Display name resolution chain: settings override → derived default → the literal `Unnamed account`. One test per branch.
- Reserved display name: `settings_update(A, display_name="Unnamed account")` raises before the DB write, as does any variant that `normalize_reference` folds onto it (case, padding, doubled space, NFKC); a row already carrying the literal still loads through `_load_settings`.
- Archive stamps and clears `archived_at`: `settings_update(A, archived=True)` stamps `archived_at` with today's date (idempotent — a later call while still archived leaves it untouched); `settings_update(A, archived=False)` clears it back to NULL.
- Inclusion / archive orthogonality: `settings_update(A, include_in_net_worth=False)` does not touch `archived`; `settings_update(A, archived=True)` does not touch `include_in_net_worth`, and an explicit `include_in_net_worth` passed in the same call as `archived=True` wins.
...
- Backward-compat delegates: `rename` / `set_include_in_net_worth` / `archive` / `unarchive` still exist as thin one-line delegates to `settings_update` for internal callers; tests of the delegates assert the `archived_at` stamp/clear is reached through the unified path.
...
- `accounts set --archive` does not print any net-worth cascade note (retired).
- `accounts set --unarchive` does not restore `include`, matching the original ergonomics; only `archived_at` clears.
...
- After archiving 2 accounts: `accounts list` count == `count - 2`; `reports.net_worth` excludes those 2 via the `archived` filter (`include_in_net_worth` is untouched by archiving).
...
3. **Archive and net-worth inclusion are independent; archiving stamps a date instead.** An archive cascade shipped originally for one-step retirement ergonomics and was retired by [`reports-net-worth-sql-surface.md`](reports-net-worth-sql-surface.md) §Prerequisites: net worth is a stock measure whose history should stay immutable, and a present-tense cascade rewrote it retroactively. `archived_at` is now recorded on the transition so a future release can carry the exclusion date-scoped instead — no report applies it yet; today `archived` alone still excludes the account from net worth entirely, via the existing blanket filter.
