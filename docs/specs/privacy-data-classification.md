...
| (app, account_settings) | archived_at | TXN_DATE | the date the account stopped being part of the position — a user-authored decision date about the account's lifecycle, not "when was this recorded" pipeline observability. Same rationale as `trade_date`/`balance_date` (see `dim_holdings.price_date`'s entry below, which draws the opposite distinction for a *market* date). Same on `core.dim_accounts`. |
...
| (core, dim_holdings) | market_value | BALANCE | quantity × a resolved close — a held "stock" figure disclosing what the position is worth, the same character as `cost_basis`/`average_cost` above it, not a single flow like a transaction amount. Same class, same tier as the cost-basis columns it sits beside. |
| (core, dim_holdings) | unrealized_gain | BALANCE | `market_value` less `cost_basis`; derivable from two BALANCE columns, so it can be no less sensitive than either. Signed, but the sign carries no class weight — realized gain/loss on `fct_realized_gains` is BALANCE for the same reason. |
...
| `reports(report_id="core:net_worth_currencies")` | Actual report route with terminal row redaction and audit bypassed only in the test raw path, vs the unchanged protected route | high | balance time-series |
