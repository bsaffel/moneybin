...
| (app, account_settings) | archived_at | TXN_DATE | the date the account stopped being part of the position — a user-authored decision date about the account's lifecycle, not "when was this recorded" pipeline observability. Same rationale as `trade_date`/`balance_date` (see `dim_holdings.price_date`'s entry above, which draws the opposite distinction for a *market* date). Same on `core.dim_accounts`. |
