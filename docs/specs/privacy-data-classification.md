...
| (core, dim_holdings) | price_source | TXN_TYPE | which provider supplied the close (`plaid`, …) — a low-cardinality routing tag, identical in kind to `fct_security_prices.source` and to the `source_type` discriminators classified TXN_TYPE elsewhere. Reveals nothing about the position itself. |
