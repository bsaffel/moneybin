...
7. **Recipe authoring on escalation.** When an unseen layout escalates to the bridge (low deterministic confidence, or a scan), the bridge proposes (a) a declarative extraction recipe (anchors, row delimiters, field order, date format, sign convention), (b) a field mapping, and (c) a routing decision (`transactions` | `seed`). A high-confidence unseen layout the deterministic rung reads cleanly proceeds without a saved recipe (Requirement 2) — the recipe is the learned artifact escalation produces, not a precondition for every first import. The recipe is human-readable text-structural rules, not pixel geometry.
...
- `src/moneybin/mcp/tools/import_tools.py` — extend `import_preview` for the PDF bridge payload (request out); register the import-family `import_confirm` tool if `smart-import-tabular.md` hasn't already (vetted recipe + rows in).
...
Reuses the existing `import_files`, `import_preview`, and `import_formats` tools, plus the import-family **`import_confirm`** (specified in `smart-import-tabular.md`; not yet registered — see Surface Design):
