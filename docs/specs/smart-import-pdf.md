...
**Phase 2 (recipes + ladder + transactions routing)** — in-progress. Adds `app.pdf_formats`, the recipe executor + replay guard, fingerprint match, the bridge export/apply seam, and transaction-shaped routing through `raw.tabular_transactions` (`source_type='pdf'`). The seed path becomes the *fallback* once routing exists.
