...
Per `.claude/rules/surface-design.md`, PDF import stays within the existing `import_*` family. It uses **`import_confirm`** for the propose→review→confirm step — the `confirm` verb (a sanctioned entry in the surface-design verb vocabulary: accept-or-override an interactively-presented proposal, the terminal step of a propose→review→confirm workflow). This is the **shared import-family tool specified in [`smart-import-confirmation.md`](smart-import-confirmation.md)** (the owner of the confirm flow, the unified confidence contract, and the `resolve_or_confirm` primitive), not a PDF-exclusive addition; it is **registered** (PR #241 introduced it alongside the tabular confirm flow), and PDF extends its payload with a `bridge_response`. The bridge payload is a data-shape addition to `import_preview`'s response and `import_confirm`'s input. Because PDF **changes `import_preview`'s contract** and exercises the not-yet-registered `import_confirm` (see MCP Interface), the implementation PR carries the surface-change obligations (`mcp.md`): update both `moneybin-mcp.md` and `moneybin-capabilities.md`. PDF introduces no new command *group* — it's fused into `import`, not a parallel surface.
...
Reuses the existing `import_files`, `import_preview`, and `import_formats` tools, plus the import-family **`import_confirm`** (specified in `smart-import-confirmation.md`; registered with its PDF-bridge branch in PR #241):
