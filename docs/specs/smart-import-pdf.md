...
**Phase 2a (deterministic recipe ladder + replay + transactions routing)** — implemented (PR #233). Adds `app.pdf_formats` (V027) with `PdfFormatsRepo` (audited per Invariant 10), the bounded recipe executor (`regex` package with per-pattern timeout), the layout-fingerprint match (`compute_fingerprint` + `match_format`, scoped to the transaction-shaped table and preserving header order), the routing state machine (`route_pdf_import` with confidence + metadata + reconciliation gates), the auto-derive path (`derive_recipe` over `pdfplumber` IR with end-anchor sentinel scan, header-line start-anchor detection, dash-placeholder exclusion, and sign-convention sanity check), and the transactions-routing wire-up in `_import_pdf_transactions`. Successful PDFs land in `raw.tabular_transactions` (`source_type='pdf'`) and feed the SQLMesh pipeline; reconciliation failures, low confidence, missing balance metadata, no-transaction-shaped-table, replay failures, and unsupported number formats all fall back to the Phase 1 seed path. (Phase 2c below narrows that fallback list: a document that *is* transaction-shaped but defeated derivation now escalates to the bridge instead of seeding.) CLI gains `--type {tabular,pdf,all}` filtering on `import formats list` and a cross-namespace `import formats show`; both surfaces read one `formats` array whose rows carry a `type` discriminator, through `import_formats` or `import_status(sections=["formats"])`. Three Prometheus metrics expose the routing decision (`pdf_extraction_confidence`, `pdf_recipe_hit_total{outcome}`, `pdf_replay_guard_failure_total`).

**Phase 2c (unruled-statement derivation + honest routing reasons)** — implemented. Corrects two defects that made the deterministic rung blind on real statements:

- **Derivation reads text lines, not just ruled tables.** `derive_recipe` selected its transaction table from `doc.tables`, which pdfplumber populates only from *drawn ruling lines* — while `execute_recipe` reads `doc.text_lines`. Real bank statements are typeset with whitespace-aligned columns and no rules, so `doc.tables` came back empty and derivation went blind on exactly the input the executor consumes: a real Chase statement with a clean `ACCOUNT ACTIVITY` section extracted **0 transactions** and landed as an opaque seed. `_select_transaction_table` now falls back to reconstructing candidate tables from `text_lines` with the same `\s{2,}` splitter the recipe executes with, so a derived recipe is self-consistent with the executor by construction. Ruled tables still win when present. Fixture `chase_checking_unruled.pdf` is the ruled twin's unruled counterpart with identical ground truth; every prior fixture drew ruling lines specifically so `extract_tables()` would find them, which is why the suite could not catch this.
- **`no_transaction_table` no longer swallows derivable-but-unparsed statements.** Routing reported *every* derivation failure as `no_transaction_table` — a reason deliberately excluded from bridge escalation on the premise that the document "isn't transaction-shaped." That premise silently buried statements that *were* transaction-shaped and merely defeated derivation. `derive_recipe` collapses all failures into `None`, so a new `derivation_failure_reason(doc)` names which one occurred, and routing splits three ways:

  | Outcome | Reason | Route | Why |
  |---|---|---|---|
  | Not a transaction document | `no_transaction_table` | seed | A brokerage positions statement; the bridge would be off-target. |
  | Transaction table, non-US number locale | `unsupported_number_format` | seed | `execute_recipe` rejects a non-US `number_format` outright, so **no** recipe the agent could author would run. Escalating would egress the user's statement text to an LLM and prompt for a confirmation, then seed anyway. |
  | Transaction table the deterministic rung can't crack | **`transaction_table_underivable`** | **bridge** | Exactly what the ladder's escalation rung is for. |

  The "is this a statement?" test (`_looks_transactional_anywhere`) is deliberately **looser** than the derivability test (`_is_transaction_shaped`): it accepts a debit/credit column pair, which `_classify_sign_convention` rejects by design (Phase 2b deferral). Those two questions are different, and answering the first with the second classed the *most common real bank layout* — separate `Withdrawals` / `Deposits` columns — as "not a transaction document" and silently seeded it. Because `\s{2,}` collapses the blank side of a debit/credit row, such a statement's rows don't even reconstruct to header width when unruled, so the predicate falls back to detecting a transaction-shaped header line with dated rows beneath it. The number-locale probe runs off that same loose table — probing the strict one would skip the check for exactly the split-column statements and escalate a European debit/credit statement the bridge could never serve.

**Known gap — the seed store can't accept an unruled document.** `write_pdf_seed` builds its rows from `doc.tables`, which pdfplumber populates only from drawn ruling lines. So a document routed to seed that has a text layer but no ruled tables — a *non-transactional unruled* PDF, e.g. a whitespace-typeset brokerage positions statement — extracts zero rows and raises `No tables extracted from PDF` rather than filing. This predates Phase 2c and is unchanged by it (before, *every* unruled document hit this; now only the ones that aren't transaction statements). Closing it means giving the seed a row shape for documents with no recoverable column structure, which is a change to the public `raw.pdf_<alias>` view contract — deliberately not decided inside a bugfix.

**Phase 2b (bridge + auto-bump + no-agent degradation)** — implemented. The bridge round-trip, recipe auto-bump, whole-ladder scenario tests, and the no-agent scanned-PDF degradation outcome all land. The in-process **LLM / vision rung** (a cloud or local backend that would read a scanned PDF without a driving agent present) remains deferred / Out of Scope (Req 5).
...
- **Bridge apply round-trip** (shipped) — `route_forced_recipe` runs an agent-authored recipe through the shared execute → confidence → reconcile engine; `ImportService.apply_pdf_bridge_response` re-executes the recipe (the reconciliation gate on the re-executed rows is the authority; the agent's returned rows are verified against them and a row-count divergence is surfaced), persists the recipe, and loads to `raw.tabular_transactions`. The parser rejects an agent recipe whose amount fields don't match its declared `sign_convention` (a mismatch sums absent reconciliation keys to 0 and would load all-zero amounts on a zero-delta statement). MCP: `import_files`/`import_preview` escalate to the driving agent (gated on `actor_kind="agent"`) and `import_confirm(preview_id=..., bridge_response=...)` applies. A `negative_is_income` bridge recipe must pass a human MCP elicitation; a client without elicitation uses `moneybin import confirm <file> --bridge-response response.json --confirm` instead.
- **Auto-bump on a stale saved recipe** (shipped, Req 9a) — when a saved recipe stops serving its layout (fails `model_validate` on replay → auto-derive; stops reconciling → bridge re-derive; reconciles but reads a digit-free account id → deterministic self-heal), the re-derived recipe is installed as a new version via `PdfFormatsRepo.bump_version` (audited, undo-reversible per Invariant 11) instead of colliding with the stale row and being skipped. Serves both the bridge apply and deterministic auto-derive paths.
- **Deterministic self-heal on a stale saved recipe** (shipped, Req 9a) — a saved recipe is a frozen snapshot of the derivation logic that authored it, so a later fix to `derive_recipe` can never reach a format already in `app.pdf_formats`: the saved copy keeps mis-parsing and every future statement of that layout seeds. `_attempt_self_heal` adds the missing rung between them — **replay → re-derive → seed** — re-deriving from the document and accepting the result only if it clears the same ±1¢ reconciliation gate a first-contact recipe must clear, then persisting it via `bump_version`. This is the no-bridge counterpart to the bridge re-derive below: it needs neither an agent nor a human, so a plain CLI import repairs itself where it previously seeded. Two guards bound the rewrite, because a recipe change the user never sees is exactly the kind of magic that must stay narrow (see *Magic stays visible* in `design-principles.md`): it declines when `source != "detected"` (never overwrite a bridge- or human-authored recipe with a machine guess — which required splitting the `source` vocabulary, since first-contact saves previously stamped `"detected"` on both rungs and the column's original semantics folded "bridge-proposed + vetted" *into* `"detected"`, making this guard a no-op for the case it names; the bridge rung now records `source="bridge"`), and a re-derived `sign_convention` that differs from the saved one is never applied on routing's own authority — the decision carries `rederived_from_sign` and `_gate_pdf_sign_convention` holds it for human ratification in **either** direction (that gate's own short-circuit exempts replays and only proposes for `negative_is_income`, so an income → expense repair would otherwise un-invert a ratified convention silently). Refusing outright was the first shape of this guard and was strictly worse: routing seeded, the gate ignores seed decisions, so no `--sign` or `--confirm` could authorize the repair and the statement became permanently unimportable. Discovered by a real Chase statement whose sub-dollar FX fee prints as `.39`: the fix that taught derivation to read it could not reach the recipe already saved.

  **Two triggers, one repair** (`_self_heal_trigger`). Reconciliation failure is the loud one. The quiet one is a replay that captures an **account id carrying no digits** — a masked account line read by an anchor that stops at the first whitespace yields the bare mask, the rows still reconcile to the cent, and the only damage is an account identity the matcher cannot bind to the same card from another source. Both share the frozen-snapshot cause, so both re-derive; they differ only in the reason recorded in the audit log, which the decision carries as `rederived_reason` rather than assuming reconciliation failed. A digit-free id is accepted as *repaired* only when the fresh derivation actually gains a digit — a statement that genuinely discloses none (a fully-masked `XXXX`) would otherwise be "repaired" into a new version and audit row on every future import while its identity never improves, the same reason a *missing* id is not a trigger at all.
- **No-agent degradation** (shipped, Req 5) — a scanned / image-only PDF with no selectable text layer returns an explicit `import_pdf_no_text_layer` unsupported outcome ("needs a vision-capable backend"), for agent and human callers alike, instead of a generic parse failure or a silent empty seed. A native-text PDF the rung can't structure still degrades to the seed path.
...
7a. **Statement-metadata capture (both rungs).** Capturing statement metadata distinct from the transaction rows — account identifier and label, account type/product, routing number, currency, statement period, and opening/closing balances — is part of *extraction*, not bridge-exclusive. The deterministic rung finds conservative labelled anchors and the bridge freezes the same captures into its recipe. These fields are evidence: complete identifiers feed Requirement 10a; labels/products and currency improve account context and review; balances drive reconciliation; routing is retained as issuer context. None becomes an independent silent-match key.
...
9. **Reconciliation gate — the primary validator, on every transaction load.** Before **any** extraction is routed to transactions — first-seen deterministic, replayed, or bridge-authored — it must pass **balance reconciliation**: the extracted rows tie to the statement's stated opening/closing balance and totals — money-in minus money-out equals `closing − opening`, computed on the **raw extracted values pre-sign-normalization**, in the statement's own terms (split debit/credit columns: `sum(credits) − sum(debits)`; a single signed-amount column: `sum(amounts)`; MoneyBin's canonical `negative = expense` sign is applied only *after* the gate passes). This is the strongest correctness signal a financial document offers, and the backstop for signal/noise separation (see *Signal vs. noise* under Architecture). Secondary checks: confidence score, row-count sanity, sign-convention. Any failure — typically a subtotal counted as a transaction or a dropped row — blocks the silent load: a replayed recipe is first **re-derived deterministically** from the document, and the fresh recipe is accepted only if it passes this same gate (the self-heal rung — see Req 9a; bounded by the source and sign-convention guards described there); failing that it re-escalates to the bridge and refreshes — or, with no bridge present, the stale replay is rejected and the document falls back to a raw-text seed (Requirement 5) rather than loading unbalanced; a first-seen deterministic extraction likewise escalates to the bridge, or, with no bridge present, is offered as a seed (Requirement 5). Documents that expose no stated totals fall back to confidence + row-count and are flagged `unreconciled`. The gate applies to the transactions route only — seeds carry no schema contract to reconcile.

9a. **Recipe versioning.** A recipe refresh — triggered by the reconciliation gate (Requirement 9) or a manual edit — creates a new **version**, never a silent overwrite. `app.pdf_formats.version` increments; the change is audited via `PdfFormatsRepo` (Invariant 10) and is reversible to a prior version through the audit-log undo (Invariant 11, per `data-recovery-contract.md`). Undo of a bad refresh restores the previous recipe version. History lives in the audit log — no separate version table. **Status:** Phase 2a landed the `PdfFormatsRepo.bump_version` primitive (manual-edit and undo paths). Phase 2b lands the **automatic** `bump_version` on a stale saved recipe: when a saved recipe stops serving its layout — it fails `model_validate` on replay (→ deterministic auto-derive), stops reconciling (→ deterministic self-heal, else bridge re-derive), or replays and reconciles while capturing an account id carrying no digits (→ deterministic self-heal, accepted only when the fresh derivation gains a digit) — the re-derived recipe is installed as a new version in place of the stale row, rather than colliding with it on the fingerprint-derived primary key and being skipped. The self-heal rung exists because a persisted recipe freezes the derivation logic that wrote it: without it, a fix to `derive_recipe` never reaches an already-saved format, and the layout stays broken forever with no signal beyond a growing seed pile. The bump is audited and undo-reversible (Invariant 11), so a bad refresh restores the prior version.

9c. **Sign convention — derive by disclosure, confirm the inversion, defer to the human.** A single signed-amount column carries no evidence of which way it points: a checking statement (`−50` groceries, `+150` paycheck) and a credit-card statement (`+150` charge, `−50` payment) have identical sign distributions, and — because the reconciliation gate (Requirement 9) sums the raw pre-normalization amounts — **both reconcile cleanly with the signs exactly backwards**. Reconciliation therefore offers zero protection here; the signal has to come from outside the numbers. It comes from the statement's own legally-required card disclosures (`minimum payment`, `credit limit`, `available credit`, `payment due date`, `annual percentage rate`). When those are present, derivation *proposes* `negative_is_income` (charges are expenses, payments are credits); when they are absent and no amount is negative, the convention is genuinely ambiguous and the document escalates rather than guessing. The inversion is **never applied silently on first contact** — it is confirmed once per statement format (`--confirm` = "yes, a card"; `--sign negative_is_expense` = "no, not a card"), because a wrong inversion corrupts every row on this import and on every future replay, and a persisted recipe replays forever. Two guards keep it honest: the replay guard is **symmetric** — a saved card (`negative_is_income`) recipe refuses to replay onto a document that no longer carries card disclosures, exactly as a saved bank recipe refuses a card — and an explicit `--sign` override that *overrules* the detector is recorded as ratified (`Recipe.sign_ratified`) so it survives replay, while an agreeing override or a `--confirm` does not disarm the guard. Ratification is a **human** act: the bridge cannot set `sign_ratified` (it is rejected at the response boundary and stripped from the recipe shown to the agent), and `import_confirm(preview_id=..., bridge_response=...)` uses the shared MCP elicitation gate or, for a client without elicitation, the explicit CLI `moneybin import confirm <file> --bridge-response response.json --confirm` fallback. A confirmed card additionally types its account `credit` (Requirement 10a), so it is negated as a liability in `core.fct_balances`. **Deliberately out of scope:** `split_debit_credit` card statements (Phase 2b — the reconciliation direction for a card's separate debit/credit columns is unsupported and declines), and trusting a bound account's type over the document scan (the account is unknown at derivation time and PDF-minted accounts carry no type until this requirement sets one — a future improvement seeded by the `credit` typing above).
...
10a. **Account resolution.** Every PDF uses an opaque document-content `source_native` key under an issuer-independent origin, so exact-file re-import is idempotent without treating issuer+last-four as identity or drifting when issuer detection changes. An identifier captured by a complete-number anchor is normalized, scoped by a validated routing number, and retained only as the existing encrypted `full_number` ref; exact equality may auto-adopt and shares the scope emitted by OFX. Issuer-only, masked, suffix-only, bridge-authored, unscoped, or otherwise unproven identifiers remain review evidence alongside statement traits and incoming transaction overlap. Legacy issuer-plus-last-four PDF links are migration candidates shown with current candidates, never strong refs. Old auto-derived recipes gain newly introduced default metadata fields on replay; bridge/manual recipe omissions remain authoritative. PDF transactions never land account-less.
...
flowchart TD
    ...
    GUARD -- pass --> IDCHK{"Replay captured an account id with no digits?"}
    IDCHK -- no --> T[raw.tabular_transactions source_type=pdf -> matching/categorization/reports]
    IDCHK -- yes --> HEAL
    GUARD -- "fail (replay of a detected recipe)" --> HEAL[Self-heal: re-derive from the document]
    GUARD -- fail --> BR
    HEAL --> HGATE{"Fresh recipe reconciles, and gains a digit if that was the trigger?"}
    HGATE -- "no (or underivable): reconciliation trigger" --> BR
    HGATE -- "no: digit-free trigger, statement still loads" --> T
    HGATE -- "yes, same sign convention" --> BUMP[bump_version: repair the saved recipe] --> T
    HGATE -- "yes, sign convention changed" --> HSIGN{Human ratifies the flip?}
    HSIGN -- confirm --> BUMP
    HSIGN -- "override (--sign)" --> BUMP
...
Per `.claude/rules/surface-design.md`, PDF import stays within the existing import workflow. It uses **`import_confirm`** for the propose→review→confirm step — the `confirm` verb (a sanctioned entry in the surface-design verb vocabulary: accept-or-override an interactively-presented proposal, the terminal step of a propose→review→confirm workflow). This is the **shared import-family tool specified in [`smart-import-confirmation.md`](smart-import-confirmation.md)** (the owner of the confirm flow, the unified confidence contract, and the `resolve_or_confirm` primitive), not a PDF-exclusive addition; it is registered and PDF extends its payload with a `bridge_response`. The bridge payload is a data-shape addition to `import_preview`'s response and `import_confirm`'s input. PDF introduces no new command *group* — it is fused into `import`, not a parallel surface.
...
Reuses the existing `import_files`, `import_preview`, and `import_status(sections=["formats"])` read, plus the import-family **`import_confirm`** (specified in `smart-import-confirmation.md`; registered with its PDF-bridge branch in PR #241):

- `import_preview` on a PDF returns the extraction outcome and, when escalation is needed, the **bridge payload**: the IR (or page image) + a structured request for the agent to propose recipe / mapping / routing. The live tool is a staged write (`readOnlyHint=false`, non-idempotent), classifies its file-derived payload dynamically, and declares maximum critical sensitivity. The bridge payload also carries the **Req 14 transparency notice** — the plain statement that proceeding surfaces the document to the driving agent; in MCP mode this is how the notice reaches the user (the agent surfaces it before calling `import_confirm`), and the audit row writes on the hand-off.
- `import_confirm` (`read_only=False`; shared import-family tool, registered) accepts the vetted recipe + rows via `bridge_response` → saves `app.pdf_formats` (audited, reversible via Invariant 11) → loads (to core or seed).
...
- **`saved_recipe_for_re_derive` is the *failing* recipe, not a proposal.** On a `replay_failed_re_derive` egress (a saved recipe stopped reconciling), the bridge payload includes the saved recipe purely as reference — the patterns the agent should inspect and refresh — alongside the document. The agent still returns a *new* recipe; the saved one is never re-applied as-is.
- **Two distinct `reason` namespaces.** Escalation (`ConfirmationRequired.reason`, in the `import_preview`/`import_files` egress envelope) uses `unknown_layout` / `validation_failure`. Apply (`BridgeApplyResult.reject_reason`, from `import_confirm`) uses the routing reason that failed re-execution (e.g. `reconciliation_failed`). They describe different lifecycle stages — egress (why we're asking) vs. apply (why the proposal was rejected) — so an agent reading both should not expect a shared vocabulary.
