...
10a. **Account resolution.** Every PDF uses an opaque document-content `source_native` key, so exact-file re-import is idempotent without treating issuer+last-four as identity. An identifier captured by a complete-number anchor is normalized, scoped by a known issuer or validated routing number, and retained only as the existing encrypted `full_number` ref; exact equality may auto-adopt. Masked, suffix-only, bridge-authored, unscoped, or otherwise unproven identifiers remain review evidence alongside statement traits and incoming transaction overlap. Legacy issuer-plus-last-four PDF links are migration candidates shown with current candidates, never strong refs. Old auto-derived recipes gain newly introduced default metadata fields on replay; bridge/manual recipe omissions remain authoritative. PDF transactions never land account-less.
