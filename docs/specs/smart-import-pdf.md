...
- **Auto-bump on replay-guard failure** (shipped, Req 9a) — when a saved recipe stops serving its layout (fails `model_validate` on replay → auto-derive, or stops reconciling → bridge re-derive), the re-derived recipe is installed as a new version via `PdfFormatsRepo.bump_version` (audited, undo-reversible per Invariant 11) instead of colliding with the stale row and being skipped. Serves both the bridge apply and deterministic auto-derive paths.
...
flowchart TD
    ...
    GUARD -- fail --> BR
    HEAL --> HGATE{Fresh recipe reconciles?}
    HGATE -- "no (or underivable)" --> BR
