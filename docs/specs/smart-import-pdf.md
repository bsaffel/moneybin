...
- **`no_transaction_table` no longer swallows derivable-but-unparsed statements.** Routing reported *every* derivation failure as `no_transaction_table` — a reason deliberately excluded from bridge escalation on the premise that the document "isn't transaction-shaped." That premise silently buried statements that *were* transaction-shaped and merely defeated derivation (unsupported number locale, ambiguous date format, ambiguous sign). Routing now distinguishes the two via `has_transaction_table(doc)`: a genuinely non-transactional document (a brokerage positions statement) keeps reporting `no_transaction_table` and seeds, while a transaction-shaped document that defeated derivation reports the new **`transaction_table_underivable`** and is bridge-eligible — the agent gets the shot the ladder was designed to give it.
