...
**Phase 2b (bridge + LLM rung + auto-bump)** — in-progress. The **bridge round-trip is implemented**: the egress payload (export) on `import_preview`/`import_files` and the apply seam (`apply_pdf_bridge_response`, wired to `import_confirm`) that re-executes the agent's recipe through the same reconcile gate before loading. **Still pending:** automatic `PdfFormatsRepo.bump_version` on replay-guard reconciliation failure (so a drifted saved recipe is re-derived through the bridge and re-installed without manual intervention), whole-ladder scenario tests, and the no-agent degradation path. Overall status stays `in-progress` until those land.
