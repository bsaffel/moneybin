...
A follow-up audit pass on the existing MCP surface (Out-of-Scope §Follow-ups) is recommended for `moneybin-mcp.md` v2 — several existing toggles are candidates for declarative-set consolidation (remaining: `accounts_include`, `accounts_archive`; `categories_toggle` was renamed to `categories_set` in the 2026-05-17 vocabulary sweep but did not need consolidation).
