...
| Category-source-mapping curation | none — `admission-pending` | `categories mappings pending`, `categories mappings set` | Same distinct unmapped imported-vocabulary terms (grouped by term, not by transaction) with suggestions, and the same map-to-existing-or-new-category-or-ignore write through `CategorizationService.resolve_source_term` |
