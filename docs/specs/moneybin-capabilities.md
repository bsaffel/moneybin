...
| 68| Confirm an inferred credit-card sign convention (charges → expenses) | `import_files`/`import_preview` return `confirmation_required` with `reason="sign_convention"` + evidence; `import_confirm` elicits a human approval before an agent-authored tabular mapping or bridge recipe can invert the ledger | `import files <path> --confirm` *(deterministic PDF)* / `import confirm <file> --accept --confirm-sign` *(tabular)* / `import confirm <file> --bridge-response response.json --confirm` *(bridge recipe)* / `--sign negative_is_expense` *(overrule a false card detection)* | — | live |
