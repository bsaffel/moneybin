...
| 67| Set up an evaluator demo profile (synthetic data → full pipeline → clean doctor → first answer) | — *(cat 2 — dev/evaluator tooling)* | `demo` *(`--persona`, `--profile`, `--seed`, `--yes`)* | — | live (CLI-only) |
