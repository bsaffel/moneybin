...
| 68| Confirm a credit-card PDF's inverted sign convention (charges → expenses) | `import_files`/`import_preview` return `confirmation_required` with `reason="sign_convention"` + `sign_sample_rows`; `import_confirm(bridge_response=...)` elicits a human approval before an agent-authored bridge recipe can invert the ledger (unsupported clients hard-fail with the CLI equivalent) | `import files <path> --confirm` *(deterministic PDF)* / `import confirm <file> --bridge-response response.json --confirm` *(bridge recipe)* / `--sign negative_is_expense` *(overrule a false card detection)* | — | live |
