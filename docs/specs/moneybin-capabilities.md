...
| 24| Bind a Google Sheet for live sync                                | `gsheet_connect` *(`url`, `adapter`, `alias`, `account_name`, `account_id`, `column_mapping`, `yes`, `accept_seed_fallback`, `no_initial_pull`; an inferred sign inversion uses inline human elicitation, never an agent parameter)* | `gsheet connect <url>` *(same mapping options; human sign choice via `--sign`)* | —          | live                  |
...
| 28| Re-detect column mapping after sheet drift                       | `gsheet_reconnect` *(`yes` for medium-confidence remaps; an inferred sign inversion uses inline human elicitation, never `yes` or another agent parameter)* | `gsheet reconnect` *(`--yes`; human sign choice via `--sign`)* | —          | live                  |
...
| 41| Execute a read-only SQL query over core/app/reports with CRITICAL columns masked (declared classes; ADR-013 for reports) | `sql_query` *(`query`)* | `sql query <sql>` *(`--output text\|json`)* | — | live |
...
| 45| Import a file and handle unknown layout via confirmation flow      | `import_files` *(returns `confirmation_required` envelope on first-encounter unknown layouts; `actions[]` contains `import_confirm` hint)* | `import files PATHS... [--confirm/--no-confirm] [--confirm-sign] [--mapping field=col]` *(a tabular sign inversion needs its own explicit confirmation; TTY: interactive prompt; non-TTY/`--output json`: envelope + exit 0)* | —          | live                  |
| 46| Confirm a proposed import column mapping                          | `import_confirm` *(`file_path`, `accept=True`, `mapping={...}`; `confirm_pdf_sign=True` for a deterministic PDF's sign inversion)* | `import confirm <file> --accept` / `--mapping field=column` | —          | live                  |
...
| 62| View current investment positions (quantity, cost basis, average cost, market value) | `investments_holdings` *(`account?`)* | `investments holdings` *(`--account`)* | — | live |
...
| 68| Confirm an inferred credit-card sign convention (charges → expenses) | `import_files`/`import_preview` return `confirmation_required` with `reason="sign_convention"` + evidence; `import_confirm` elicits a human approval before a tabular mapping, a bridge recipe, or a deterministic PDF (`confirm_pdf_sign=True`) can invert the ledger | `import files <path> --confirm` *(deterministic PDF)* / `import confirm <file> --accept --confirm-sign` *(tabular)* / `import confirm <file> --bridge-response response.json --confirm` *(bridge recipe)* / `--sign negative_is_expense` *(overrule a false card detection)* | — | live |
...
the transactions. A recipe with an inferred ledger-wide sign inversion now
requires MCP human elicitation; the explicit CLI
`import confirm <file> --bridge-response response.json --confirm` path is the
fallback for clients that cannot elicit. The initial bridge escalation remains
gated on the agent caller (`actor_kind="agent"`).
