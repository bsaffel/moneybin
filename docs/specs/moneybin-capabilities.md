...
| 46| Confirm a proposed import column mapping                          | `import_confirm` *(`file_path`, `accept=True`, `mapping={...}`; `confirm_sign=True` for a deterministic PDF's sign inversion)* | `import confirm <file> --accept` / `--mapping field=column` | —          | live                  |
...
| 68| Confirm an inferred credit-card sign convention (charges → expenses) | `import_files`/`import_preview` return `confirmation_required` with `reason="sign_convention"` + evidence; `import_confirm` elicits a human approval before a tabular mapping, a bridge recipe, or a deterministic PDF (`confirm_sign=True`) can invert the ledger | `import files <path> --confirm` *(deterministic PDF)* / `import confirm <file> --accept --confirm-sign` *(tabular)* / `import confirm <file> --bridge-response response.json --confirm` *(bridge recipe)* / `--sign negative_is_expense` *(overrule a false card detection)* | — | live |
