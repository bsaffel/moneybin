...
- The review surface for ambiguous matcher decisions uses `reviews(kind="matches", status="pending")` and `reviews_decide(decisions=[...])` in MCP, and `moneybin transactions matches pending` in the CLI. Specified in [`matching-nway-dedup.md`](matching-nway-dedup.md) Requirements 11–12.
...
|---|---|---|---|---|
| **Transaction** (transaction-grain claim) | `raw.tabular_transactions`, `raw.ofx_transactions`, `raw.plaid_transactions` — each row carries `source_type` and `source_origin`; the institution-assigned identity column varies by source (`source_transaction_id` on OFX from the FITID element and on tabular when the file carries one; `transaction_id` on Plaid per Plaid's globally-unique ID) | `meta.fct_transaction_provenance` — one row per (canonical_txn, contributing_raw_row); rebuilt deterministically by SQLMesh from `app.match_decisions` joined to the contributing `raw.*` rows | `app.match_decisions` — pairwise edges with `match_status ∈ {pending, accepted, rejected}` plus `reversed_at`; auto-merge ≥0.95, review queue 0.70–0.95, drop <0.70; mutations routed through `MatchDecisionsRepo` per Invariant 10 | `core.fct_transactions` (canonical row per real-world transaction, with `source_type` — aliased from internal `canonical_source_type` in the SQLMesh model — plus `source_count`, `match_confidence`, `is_transfer`, `transfer_pair_id`). Review surface: `reviews(kind="matches", status="pending")` / `reviews_decide(decisions=[...])` MCP, `moneybin transactions matches pending` CLI |
| **Balance** (point-in-time balance claim) | `prep.stg_ofx__balances` (institution statement balances) and `prep.stg_tabular__transactions.balance` (CSV/TSV/Excel/Parquet running-balance columns). Plaid balance snapshots are a tracked extension (see [Genuine gaps](#known-follow-ups)) | `core.fct_balances` (VIEW) unions the per-source claims (OFX, tabular) plus `app.balance_assertions` from the curation column into a normalized shape `(account_id, balance_date, balance, source_type, source_ref)` — Plaid balance snapshots are not yet wired in (see [Genuine gaps](#known-follow-ups) §1); precedence within a day: user assertion > institution snapshot > tabular running balance | `app.balance_assertions` — user-authored anchors; mutations routed through `BalanceAssertionsRepo` per Invariant 10 | `core.fct_balances_daily` (TABLE; daily carry-forward with `is_observed`, `observation_source`, `reconciliation_delta`); `reports.net_worth` (cross-account aggregate). Drift surface: `accounts_balances(view="reconcile", ...)` MCP, `moneybin accounts balance reconcile` CLI |
| **Non-canonical payload** (PDF summary block, freeform sheet, etc.) | `raw.pdf_seeds` (JSON) plus auto-generated typed views per [`connect-gsheet.md`](connect-gsheet.md) seed pattern; lineage via `(alias, row_hash, source_file, page, import_id, loaded_at)` carried on the raw row, with `import_id` foreign-keying to `raw.import_log` | No separate lineage layer — the raw row is the observation, queried directly via the generated view or `sql_query` MCP tool | None today (read-only payload; no curation surface needed until a consumer requires one) | Queryable via the generated typed view or the `sql_query` tool. Not pivoted into a canonical `core.*` fact unless and until a consumer requires it |
...
1. **Matcher-pending edges** — surfaced via `reviews(kind="matches", status="pending")` MCP and `moneybin transactions matches pending` CLI:
    - (a) Same-record-dedup pairs at `0.70 ≤ confidence < 0.95` (sub-0.70 drop; ≥0.95 auto-merge per [`matching-overview.md`](matching-overview.md)).
    - (b) Transfer-detection candidates at `confidence ≥ transfer_review_threshold` (default 0.55, always-review in v1 per [`matching-transfer-detection.md`](matching-transfer-detection.md) Req 3 — no auto-merge path for transfers).
2. **Balance reconciliation drift** — per-day deltas where transactions do not account for the change between consecutive anchors. Surfaced via `accounts_balances(view="reconcile", ...)` MCP and `moneybin accounts balance reconcile` CLI. Per [Genuine gaps](#known-follow-ups), the interval-level "anchor-to-anchor drift = net transactions" audit is a tracked extension to [`moneybin-doctor.md`](moneybin-doctor.md).
...
| Pending review items (matcher couldn't decide) | `reviews(kind="matches", status="pending")` MCP tool or `moneybin transactions matches pending --output json` CLI. Both return the `ResponseEnvelope` per [`architecture-shared-primitives.md`](architecture-shared-primitives.md), over the same `MatchesPendingPayload` — the rows are at `.data.matches`, and `.data.n_dedup_groups` counts the components across the whole queue rather than the returned page. |
