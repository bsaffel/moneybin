...
1. **Matcher-pending edges** — fuzzy same-record-dedup pairs in the 0.70–0.95 band (sub-0.70 dedup pairs drop; ≥0.95 dedup pairs auto-merge per [`matching-overview.md`](matching-overview.md)) AND every transfer-detection candidate above the `transfer_review_threshold` (default 0.55, always-review in v1 per [`matching-transfer-detection.md`](matching-transfer-detection.md) Req 3 — no auto-merge path). Surfaced via `transactions_matches_pending` MCP and `moneybin transactions review --type matches` CLI.
