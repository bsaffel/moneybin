...
- The review surface for ambiguous matcher decisions lives in `transactions_matches_pending` / `transactions_matches_set` MCP tools and `moneybin transactions review --type matches` CLI. Defined in [`matching-overview.md`](matching-overview.md) §"Match review UX".
...
- Audit and reversibility on every mutation to `app.match_decisions` and `app.balance_assertions` are covered by Invariant 10 (mutation routing through `*Repo` classes that emit paired `app.audit_log` rows in the same DuckDB transaction) per [`app-integrity-invariant.md`](app-integrity-invariant.md), and Invariant 11 (recoverability of mutations via `system_audit_undo`) per [`data-recovery-contract.md`](data-recovery-contract.md).
...
- **No `core.fct_balance_anchors`** — its job is `core.fct_balances`. The vocabulary "balance anchor" already maps onto rows of `fct_balances`; renaming or duplicating the table only fragments the read path.
...
|---|---|
| Per-source transaction rows visible to the user (the "bank ledger" pane) | `raw.tabular_transactions` / `raw.ofx_transactions` / `raw.plaid_*` filtered by `account_id` and date range. Use `source_type` for source attribution. Soft-deleted upstream rows carry `deleted_from_source_at` (per [`connect-gsheet.md`](connect-gsheet.md)) — exclude or surface depending on the view. |
...
| Bank-only rows (matcher saw nothing to collapse into; survived as their own canonical) | `core.fct_transactions WHERE source_count = 1`, optionally filtered by `canonical_source_type`. |
...
1. **Plaid balance sync into `core.fct_balances`.** Listed as a future extension in [`reports-net-worth.md`](reports-net-worth.md) Req 1. Amend [`sync-plaid.md`](sync-plaid.md) to write a `prep.stg_plaid__balances` view that `fct_balances` unions in. Cost-of-delay is moderate — the user can already assert balances manually — but landing it removes the largest blind spot in automated balance drift detection.
