...
1. **Matcher-pending edges** — surfaced via `reviews(kind="matches", status="pending")` MCP and `moneybin transactions review --type matches` CLI:
...
| Pending review items (matcher couldn't decide) | `reviews(kind="matches", status="pending")` MCP tool or `moneybin transactions review --type matches --output json` CLI. Same `ResponseEnvelope` shape per [`architecture-shared-primitives.md`](architecture-shared-primitives.md). |
