...
| Pending review items (matcher couldn't decide) | `reviews(kind="matches", status="pending")` MCP tool or `moneybin transactions matches pending --output json` CLI. Same `ResponseEnvelope` shape per [`architecture-shared-primitives.md`](architecture-shared-primitives.md). |
