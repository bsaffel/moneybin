...
| Pending review items (matcher couldn't decide) | `reviews(kind="matches", status="pending")` MCP tool or `moneybin transactions matches pending --output json` CLI. The MCP tool returns the `ResponseEnvelope` per [`architecture-shared-primitives.md`](architecture-shared-primitives.md); the CLI command emits `{"matches": [...]}` and does not carry the envelope yet. |
