...
| Windsurf | stdio, streamable HTTP, SSE (OAuth on all) | `mcp_config.json`; in-app marketplace | **T2** | Works via stdio, but its 100-active-tool cap creates a per-release headroom tax against MoneyBin's 105-tool surface. Document only; revisit if the cap changes or a supported install path removes the tax |
