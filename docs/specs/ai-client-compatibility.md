...
| Windsurf | stdio, streamable HTTP, SSE (OAuth on all) | `mcp_config.json`; in-app marketplace | **T2** | Works via stdio. Its 100-active-tool global cap accommodates all 45 MoneyBin tools and leaves 55 tool slots for other servers. Document only; revisit if momentum warrants release-gated support |
...
- **Windsurf → T2.** Its 100-active-tool global cap accommodates all 45
  MoneyBin tools and leaves 55 tool slots for other servers. Stdio
...
5. **Keep Windsurf headroom current.** The full 45-tool standard registry is
   visible at connect. Cascade's hard 100-active-tool global ceiling therefore
   leaves 55 tool slots for every other connected server. Derive the MoneyBin
...
- **Windsurf → T2** (was OQ1). Its 100-active-tool global cap accommodates all
  45 MoneyBin tools and leaves 55 tool slots for other servers. The client
