...
>
> **Privacy:** Anthropic does not train on API data. Columns classed as account or routing numbers are always masked. Spreadsheet and importer columns in `raw`/`prep` are scanned by value instead, which misses an account number shorter than 8 digits.
>
