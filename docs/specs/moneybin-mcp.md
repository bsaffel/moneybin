...
- **Confirmation binds to the approved bytes.** On every channel the file's SHA-256 is captured when the proposal is raised and re-checked after the human answers; if the file changed while the prompt was open, the tool raises `file_changed_during_confirmation` and imports nothing. Without this, an approval granted for one statement would silently transfer to whatever replaced it — pre-ratifying an unseen inversion.
