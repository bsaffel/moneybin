...
`bridge_response`, and `confirm_sign` — are mutually exclusive.
...
- **Behavior (sign convention):** all three channels route an inferred inversion to the same human gate — a tabular mapping accepted by an agent, a `negative_is_income` bridge recipe, and a deterministic PDF called with `confirm_sign=True` each pause for shared MCP elicitation naming the evidence and printed-vs-recorded samples. Only an accepted human response retries with the sign gate open; a declined, cancelled, or elicitation-incapable client gets `mutation_confirmation_required` and **nothing loads**. `accept=`/`mapping=` still never ratify a PDF — they are the tabular column-mapping signal, and a PDF sent with them is refused with `confirm_channel_conflict` naming `confirm_sign=True`. The MCP `import_confirm` contract deliberately has no `sign` input and cannot self-confirm an inversion on any path. Explicit *override* of a false card detection stays CLI-only, with parity across both entry points: `moneybin import files <path> --sign negative_is_expense` and `moneybin import confirm <path> --accept --sign negative_is_expense`.
