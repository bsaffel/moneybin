...
- **Behavior (sign convention):** `accept=`/`mapping=` never ratify a deterministic PDF. A tabular mapping accepted by an agent pauses for shared MCP human elicitation before an inferred sign inversion can load. Deterministic imports use `moneybin import files <path> --confirm` / `--sign negative_is_expense`; bridge recipes use MCP elicitation or the CLI `moneybin import confirm <path> --bridge-response response.json --confirm` fallback. The agent cannot silently reverse a statement on any path.
...
- **CLI:** `moneybin import confirm <file> [--accept] [--confirm-sign] [--mapping field=column] [--bridge-response response.json --confirm] [--save-format/--no-save-format] [--account-name NAME] [--account-id ID] [--account-binding source_key=ACCOUNT_ID|new] [--account-meta source_key:field=value] [--output text|json]`
