...
| **Elicitation** | optional | ✅ shipped (first-run setup; bridge and tabular sign confirmation) | First use: `FirstRunSetupMiddleware` elicits a profile name on the first tool call when the server booted unconfigured (see "First-run setup" above). Capability probed via `session.check_client_capability`; tools-only clients get the `infra_setup_required` envelope fallback. A second use is in-place confirmation of an inferred tabular or bridge sign inversion during import (see [`import_confirm`](#import_confirm)); an agent can never self-accept it. Still NOT used for general destructive-write confirmation — that stays on the `confirm` parameter pattern (revisit when destructive write-tool count > 5). |
