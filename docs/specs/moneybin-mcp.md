...
| **Elicitation** | optional | ✅ shipped (first-run setup; bridge, tabular, and Google Sheets sign confirmation) | First use: `FirstRunSetupMiddleware` elicits a profile name on the first tool call when the server booted unconfigured (see "First-run setup" above). Capability probed via `session.check_client_capability`; tools-only clients get the `infra_setup_required` envelope fallback. Sign confirmation uses in-place human elicitation for inferred tabular or bridge inversions during import (see [`import_confirm`](#import_confirm)) and for inferred Google Sheets inversions during `gsheet_connect` / `gsheet_reconnect`; an agent can never self-accept one. Still NOT used for general destructive-write confirmation — that stays on the `confirm` parameter pattern (revisit when destructive write-tool count > 5). |
...
- **Behavior:** Validates each path, delegates to `ImportService.import_files()`. On success, returns envelope with `data.{imported_count, failed_count, total_count, transforms_applied, transforms_duration_seconds, files: list[{path, status, source_type, rows_loaded, import_id, error?, error_code?, hint?}]}`. Amounts use accounting convention: negative=expense, positive=income; transfers exempt.

  **Failed-file detail.** A failed entry's `error` carries the *classified* message from `classify_user_error` (sanitized by construction — e.g. `"Permission denied: /path/to/statement.csv"`) with `error_code` set to the matching stable code (`infra_permission_denied`, `infra_file_not_found`, …) and `hint` carrying that classification's actionable recovery advice (the `chmod`/`chown` suggestion on a mode denial, the macOS Full-Disk-Access walkthrough on a TCC block). An exception the classifier doesn't recognize keeps `error` as the bare exception class name with `error_code` and `hint` both absent: `str(e)` on an unclassified extractor error can embed row content, so unclassified failures stay deliberately opaque and carry no advice.

  **Total-failure status.** Partial success is genuine success — a batch where at least one file imported (or is awaiting confirmation) returns `status="ok"`. A batch where *every* file failed returns `status="error"` with `error.code` and `error.hint` taken from the first failed entry and a message naming how many files failed rather than hoisting one file's cause. When that first entry is unclassified it has no code of its own, so `error.code` falls back to `import_parse_error` (the domain is right and the claim is narrow — the file could not be processed) and `error.hint` is omitted. The `files[]` payload is preserved so the caller still sees each file's own `error`/`error_code`/`hint`.
...
- **Unique parameters:**
  ...
  | `bridge_response` | `dict \| None` | **PDF:** the agent's `{recipe, rows}` reply. Mutually exclusive with `accept`/`mapping`. A `negative_is_income` recipe prompts the human through MCP elicitation; an unsupported client gets `mutation_confirmation_required` and the CLI equivalent. |
...
- **Behavior (sign convention):** `accept=`/`mapping=` never ratify a deterministic PDF. A tabular mapping accepted by an agent pauses for shared MCP human elicitation before an inferred sign inversion can load. Explicit tabular recovery is CLI-only and has parity across both entry points: `moneybin import files <path> --sign negative_is_expense` and `moneybin import confirm <path> --accept --sign negative_is_expense`. The MCP `import_confirm` contract deliberately has no `sign` input and cannot self-confirm an inversion. Bridge recipes use MCP elicitation or the CLI `moneybin import confirm <path> --bridge-response response.json --confirm` fallback. The agent cannot silently reverse a statement on any path.
