...
| **Elicitation** | optional | ✅ shipped (first-run setup; bridge, tabular, and Google Sheets sign confirmation) | First use: `FirstRunSetupMiddleware` elicits a profile name on the first tool call when the server booted unconfigured (see "First-run setup" above). Capability probed via `session.check_client_capability`; tools-only clients get the `infra_setup_required` envelope fallback. Sign confirmation uses in-place human elicitation for inferred tabular, bridge, or deterministic-PDF inversions during import (see [`import_confirm`](#import_confirm)) and for inferred Google Sheets inversions during `gsheet_connect` / `gsheet_reconnect`; an agent can never self-accept one. Still NOT used for general destructive-write confirmation — that stays on the `confirm` parameter pattern (revisit when destructive write-tool count > 5). |
...
- **Behavior:** Validates each path, delegates to `ImportService.import_files()`. On success, returns envelope with `data.{imported_count, failed_count, total_count, transforms_applied, transforms_duration_seconds, files: list[{path, status, source_type, rows_loaded, import_id, error?}]}`. Amounts use accounting convention: negative=expense, positive=income; transfers exempt.
  ...
  **`confirmation_required` state (first-encounter unknown layout):** instead of importing, returns a `ResponseEnvelope` with `status="ok"`, `summary.status="confirmation_required"`, and per-file `confirmation_payload`. The `actions[]` field contains concrete invocation hints for `import_confirm`. The caller inspects the proposal (optionally via `import_preview`) and calls `import_confirm` to ratify. Sensitivity is `medium` for this state because the envelope contains row-level content.
  ...
  - **PDF bridge** (Phase 2b): a native-text PDF the deterministic rung can't crack escalates to the driving agent instead of silently seeding (gated on the agent caller — `actor_kind="agent"`). `confirmation_payload.{channel="pdf", bridge_payload}` carries the document text + table preview + layout fingerprint + transparency notice + `request_kind`; the agent proposes a recipe + rows and calls `import_confirm(bridge_response=...)`. If the recipe would invert every amount, MoneyBin elicits explicit human approval before loading. With no agent present, the PDF falls back to the Phase 2a seed path.
  - **Sign convention** (credit-card inference): a PDF or tabular export can derive `negative_is_income`, which inverts every amount's sign (charges → expenses, payments → credits). Because the convention is not recoverable from the numbers, it is never applied silently. `confirmation_payload.{reason="sign_convention", sign_convention, sign_prior_convention, sign_evidence, sign_sample_rows}` carries the evidence and samples available. `sign_prior_convention` is the convention being *replaced* — non-null only when a self-healed recipe (Req 9a) re-derived to the opposite polarity, and null on a first-contact inference. It exists because the two cases pose different questions: with no prior, the proposal is always `negative_is_income` and "is this a credit card?" is the honest framing; with one, the flip can run either direction, so the `actions[]` name both conventions and what each does instead. The agent must NOT ratify it: `import_confirm` uses the shared MCP elicitation gate on all three channels — tabular mappings, bridge recipes, and deterministic PDFs (`confirm_pdf_sign=True`); clients without that capability hard-fail with the matching CLI equivalent. A replay of an already-confirmed format loads without re-asking.
- **Service:** `ImportService.import_files() -> BatchImportResult | ConfirmationRequiredResult`
- **CLI:** `moneybin import files PATHS... [--confirm/--no-confirm] [--confirm-sign] [--mapping field=column] [--no-refresh] [--output json]`

Per-file overrides (`account_name`, `institution`, `format_name`) are not exposed on the batch MCP surface — use the CLI for those.

### `import_confirm`

Terminal `_confirm` step of the propose→review→confirm workflow. Three channels —
**tabular** (ratify a detected column mapping), **PDF bridge** (apply an
agent-authored extraction recipe), and **PDF sign** (resolve a deterministic
PDF's inferred credit-card inversion). `import_files`/`import_preview` returns a
`confirmation_required` envelope; the agent inspects the proposal and calls
`import_confirm` to ratify. The three channel signals — `accept`/`mapping`,
`bridge_response`, and `confirm_pdf_sign` — are mutually exclusive.
...
- **Unique parameters:**
  ...
  | `bridge_response` | `dict \| None` | **PDF:** the agent's `{recipe, rows}` reply. Mutually exclusive with `accept`/`mapping`/`confirm_pdf_sign`. A `negative_is_income` recipe prompts the human through MCP elicitation; an unsupported client gets `mutation_confirmation_required` and the CLI equivalent. |
  | `confirm_pdf_sign` | `bool` | **PDF (deterministic):** enter the sign-inversion resolution for a PDF flagged `reason="sign_convention"`. Default `False`. Deterministic PDFs only — mutually exclusive with `bridge_response`/`accept`/`mapping`, and rejected with `confirm_channel_conflict` on a tabular file. Tabular account signals (`account_name`, `account_bindings`, `account_metadata`) are rejected with `pdf_account_signal_unsupported` — as on the bridge channel, since both bottom out in a service method taking only `account_id`. PDF rows resolve their account from the statement; pin with `account_id`. It does **not** ratify the inversion: it asks MoneyBin to elicit the human, who approves or declines. The proposal is re-derived via the non-mutating `pdf_preview` path before anything is written, so a file with no pending confirmation raises `sign_confirmation_not_pending` instead of being imported unasked. **Deliberately not named `confirm_sign`:** the CLI's pre-existing `--confirm-sign` is a different, non-overlapping thing — a human *asserting* approval of a **tabular** inversion, passed *with* `--accept`. MCP has no equivalent and cannot have one (a parameter asserting approval would let the agent self-accept); its approval always comes from the elicitation. The `_pdf_` infix keeps the two from reading as the same switch across surfaces. |
  ...
  | `account_metadata` | `dict[str, dict[str, str]] \| None` | For `"new"`-bound accounts: `source_account_key` → `{display_name, account_subtype, last_four, currency_code}` captured into the minted account's `app.account_settings`. Unknown fields raise; ignored for adopted accounts. |
...
- **Behavior (PDF bridge):** Delegates to `ImportService.apply_pdf_bridge_response()` — re-runs the agent's recipe against the document, reconciles the **re-executed** rows against the statement balances (the authority; the agent's returned rows are verified against them and a row-count divergence is reported back), persists the recipe, and loads the transactions. A `negative_is_income` recipe pauses for a shared MCP elicitation that names the evidence and printed-vs-recorded samples; only an accepted human response retries the same recipe with the sign gate open. A declined, cancelled, or unsupported client receives `mutation_confirmation_required` and writes nothing. Returns `data.{status="applied", import_id, rows_loaded, format_name, expected_row_count, actual_row_count, rows_diverged}`. A response whose re-executed rows fail reconciliation is rejected: `data.{status="invalid", reject_reason, …}` and **nothing loads**. A malformed response or an out-of-bounds recipe (Req 9b) returns a `bridge_response_invalid` error envelope.
- **Confirmation is checked against the file it was raised for.** On every channel the file's SHA-256 is captured when the proposal is raised and re-checked after the human answers; if it changed while the prompt was open, the tool raises `file_changed_during_confirmation` and imports nothing. This closes the human-decision window — seconds to minutes wide, and reachable by ordinary behavior such as re-downloading a corrected statement to the same path — where an approval would otherwise transfer to content nobody reviewed. It is a check-then-open, not a snapshot: the microsecond gap between the re-check and the importer's own open is not covered, which would require a local process that can already write the user's files and watch their reads.
- **Behavior (sign convention):** all three channels route an inferred inversion to the same human gate — a tabular mapping accepted by an agent, a `negative_is_income` bridge recipe, and a deterministic PDF called with `confirm_pdf_sign=True` each pause for shared MCP elicitation naming the evidence and printed-vs-recorded samples. Only an accepted human response retries with the sign gate open; a declined, cancelled, or elicitation-incapable client gets `mutation_confirmation_required` and **nothing loads**. `accept=`/`mapping=` still never ratify a PDF — they are the tabular column-mapping signal, and a PDF sent with them is refused with `confirm_channel_conflict` naming `confirm_pdf_sign=True`. The MCP `import_confirm` contract deliberately has no `sign` input and cannot self-confirm an inversion on any path. Explicit *override* of a false card detection stays CLI-only, with parity across both entry points: `moneybin import files <path> --sign negative_is_expense` and `moneybin import confirm <path> --accept --sign negative_is_expense`.
...
- **CLI:** `moneybin import confirm <file> [--accept] [--confirm-sign | --sign negative_is_expense|negative_is_income|split_debit_credit] [--mapping field=column] [--bridge-response response.json --confirm] [--save-format/--no-save-format] [--account-name NAME] [--account-id ID] [--account-binding source_key=ACCOUNT_ID|new] [--account-meta source_key:field=value] [--output text|json]`
...
| `gsheet_reconnect` [`connection_id`, `yes`] | medium | Re-detect sheet structure, re-pin the column mapping, run a pull. Use after drift_detected. Medium-confidence remaps require `yes=True` (symmetric to `gsheet_connect`); `yes` accepts only the mapping and never confirms sign. An inferred whole-ledger sign inversion pauses for inline human elicitation and retries once internally only after acceptance; neither `sign` nor the private retry flag is exposed in the MCP schema. The CLI equivalent keeps `--sign` for a human's explicit choice. Its 180s timeout gives that human decision the same headroom as connect/OAuth. Mutation surface: `app.gsheet_connections.column_mapping` + `header_signature` (audited). No revert — the connection re-binds to the sheet's current shape. |
