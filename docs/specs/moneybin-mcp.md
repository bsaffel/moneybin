...
- **Behavior (PDF sign convention):** `accept=`/`mapping=` never ratify a PDF. Deterministic imports use the existing CLI `moneybin import files <path> --confirm` / `--sign negative_is_expense` recovery. Bridge recipes use MCP elicitation when the client supports it, or the CLI `moneybin import confirm <path> --bridge-response response.json --confirm` fallback. The agent cannot silently reverse a statement on either path.
...
- **CLI:** `moneybin import confirm <file> [--accept] [--mapping field=column] [--bridge-response response.json --confirm] [--save-format/--no-save-format] [--account-name NAME] [--account-id ID] [--account-binding source_key=ACCOUNT_ID|new] [--account-meta source_key:field=value] [--output text|json]`
