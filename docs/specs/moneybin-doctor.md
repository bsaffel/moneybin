...
| The two rows are already in one component | component | `assign_components` skips an edge on `find(a) == find(b)`. Components are the transitive closure of `accepted`/`pending` dedup edges, keyed on the full node identity (`account_id`, `source_type`, `source_origin`, `source_transaction_id`); a row in no component is a component of one |
...
The `account_id` correlation on the node identity is load-bearing. A source-native id is unique only within its account (`identifiers.md`), so an un-namespaced FITID reused by a second account would otherwise mark a row "already decided" and suppress the warning — most likely on two accounts at one institution, which is exactly the pair an account-link merge just joined.

**The component mirror is close, not exact.** `assign_components` registers physical sources only for the nodes appearing in a given run's candidate list, and that per-run state is never persisted. The SQL registers every row carrying a `source_file`, so it can suppress where a seed-only node supplies the shared triple. That is the deliberate direction: extra silence in a `warn` costs a missed pair, while a warning no refresh can clear trains users to ignore `doctor` entirely.
