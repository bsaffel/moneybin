...
**The component mirror is close, not exact.** `assign_components` registers physical sources only for the nodes appearing in a given run's candidate list, and that per-run state is never persisted. The SQL registers the same way — the endpoints of the pairs the blocking join produces — so a seed-only member of a component contributes no `source_file` and cannot suppress a pair the matcher would go on to propose. What remains approximate is one step further in: the matcher registers the *scored* subset of those pairs, so the guard can still see a file a given run would not have. That residue is the deliberate direction: extra silence in a `warn` costs a missed pair, while a warning no refresh can clear trains users to ignore `doctor` entirely.
