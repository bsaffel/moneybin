...
7. **Doctor recipe registry.** Each invariant audit (SQLMesh named + DoctorService extra) ships with a Python recipe that produces `recovery_actions` from `affected_ids` and context:
    ...
    | `orphan_app_state` (new) | (1) `transactions_annotate(requests=[{"kind":"note_set","transaction_id":...,"note":null}, ...])` and/or empty `tags_set` requests per orphan transaction. `confidence=certain` |
...
13. **`transactions_annotate` repairs orphan collections declaratively.** A
    null `note_set` or empty `tags_set` may clear existing app rows when the
    transaction is absent from core. Non-empty targets and empty requests with
    no existing app state still fail resolution, so the exception cannot create
    orphan state or turn a bad identifier into a successful no-op. Multiple
    cleanup requests share the existing atomic batch; no recovery-only tool is
    added.
...
- New audit + recipe: `orphan_app_state` — scans
  ...
  IDs absent from `fct_transactions`. The recipe produces declarative
  clear-to-empty annotation requests per orphan transaction.
...
- **Prefixed `affected_ids` for `orphan_app_state`.** The audit emits
  `note:<transaction_id>` and `tag:<transaction_id>` so the recipe can dispatch
  by entity type without re-querying the DB. Multiple rows of either kind on
  the same orphan transaction collapse to one entry. The recipe clears each
  collection through `transactions_annotate` with a `note_set` null target or
  an empty `tags_set`; the service permits these empty target states for an
  absent canonical transaction but still rejects any attempt to create orphan
  state. Unknown prefixes and empty-id edge cases log a warning rather than
  silently producing zero actions.
- **Orphan note/tag actions are `confidence="certain"` and idempotent.** Both
  are declarative clear-to-empty operations through `transactions_annotate`.
...
- Extend the declarative annotation service to clear existing note/tag
  collections for transaction IDs absent from core, while rejecting non-empty
  targets and absent-state no-ops.
- Update `orphan_app_state` to emit schema-valid `transactions_annotate`
  requests keyed by orphan transaction ID.
