...
- **`transactions_notes_delete` cited single-id form for now.** PR 8 will add a list form (`note_ids: list[str]`). Until then the recipe emits one action per orphan note id against the existing single-id tool. No deprecation marker needed; the recipe is the only caller that would migrate to the list form.
- **Recipe scope: 6 recipes (`orphan_app_state` + 5 from the spec table).** PR 4 ships recipes only for audits where the action is genuinely executable today: `orphan_app_state` (certain delete actions), `categorization_coverage` (certain `transactions_categorize_run`), `dedup_reconciliation` (suggested `refresh_run(steps=["match"])` + `system_doctor(full=True)`), and the three SQLMesh audits — `fct_transactions_fk_integrity`, `fct_transactions_sign_convention`, `bridge_transfers_balanced` — each emitting one suggested `system_doctor(full=True)`. The spec's `accounts_get` / `import_revert` / `transactions_matches_set` citations defer to PR 9a–f (per-domain retrofit), where the audit's affected_ids will be augmented with the IDs those tools need (account_id from a transaction, source import_id, match_id from a transaction). Today's affected_ids are `transaction_id` values and the named tools take other identifiers, so emitting them now would produce non-executable actions.
- **Audits without a registered recipe leave `recovery_actions=None`.** Per the prompt's "don't fabricate certain recipes" guidance, the per-table `app_audit_coverage_*` checks (13 of them), the FK-style app audits, and the `app_user_categories_uniqueness` / `app_user_merchants_orphans` warnings have no registered recipe yet — their cleanest recovery requires the per-domain retrofit landing in PR 9. They stay surfaced as failures with an empty `recovery_actions` field (rather than spurious "investigate manually" hints) so an agent can route them to the operator rather than burning tokens on a useless action.
