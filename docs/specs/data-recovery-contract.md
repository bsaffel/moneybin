...
12. **Refuse a restore the write path would now reject, not a `CHECK`-constraint rebuild (#547).** `BaseRepo.undo_event` reversed a DELETE/UPDATE by writing the captured before-image back verbatim, with no validator in the path — so undoing a pre-#517 mutation could resurrect a whitespace-only category/subcategory the write path has refused since #517, on every table with a category/subcategory-shaped text column: `user_categories`, `user_merchants`, `transaction_splits`, `budgets`, `categorization_rules`, `proposed_rules`, `transaction_categories`, `rule_conflicts`, and `categorization_decisions` — found by sweeping every `BaseRepo` table's schema for such a column rather than trusting the issue's initial (incomplete) list of six. Two options were weighed: (a) refuse the restore with a classified error (`undo_value_inadmissible`), or (b) rebuild those tables with a schema-level `CHECK` so no path — write or restore — can produce the value at all. `CHECK` was rejected on cost: DuckDB has no `ALTER TABLE ADD CONSTRAINT`, so adding one to an existing table needs a full create/copy/drop/rename per table (the cost #517 already declined to pay for a much smaller ask), and it restates the blank-text rule a second time in SQL rather than reusing the one Python function (`validate_category_text`) the write path already calls. Refusing is a code-only change in `BaseRepo._require_admissible` that calls that same function, so the rule stays declared in exactly one place. Accepted cost: the specific audited operation becomes permanently un-undoable through this path — a narrower instance of the same trade-off already accepted for `import_revert` (see Out of Scope above): the refusal names the offending field and value so the row is not a dead end, and the other captured fields stay inspectable via `system_audit(view="detail")` for manual recreation through the normal write path.
13. **Two-step migration for `operation_id` backfill, not a column-reference DEFAULT.** Drafted with `ADD COLUMN operation_id TEXT NOT NULL DEFAULT 'op_legacy_' || audit_id`; the spec was updated during review (2026-05-20, PR #188 — Codex P1 + Claude review) to use the standard add-then-UPDATE-then-SET-NOT-NULL pattern. Reason: DuckDB (and standard SQL) does not allow column references inside `DEFAULT` clauses — that is a `GENERATED ALWAYS AS` feature, not a default. The original SQL would error at migration time and block adoption on every existing database. The four-step migration (ADD nullable → UPDATE backfill → ALTER NOT NULL → CREATE INDEX) is the deterministic safe path.
