...
mcp = FastMCP(
    ...
    instructions=textwrap.dedent(
        ...
        Money amounts are JSON numbers. summary.display_currency names the currency of the rows in that response when they all share one; it is null when they span more than one currency or the currency is unknown, and each row's own currency_code is then the authority. It describes the returned rows only — when summary.has_more is true, later pages may carry other currencies. Never add amounts whose currency_code differs; pass display_currency to reports() to price its rows into one currency instead, which defaults to the currency the user treats as home. Stored rates are gathered into the home currency, so asking for any other target usually falls back to the original currencies until that target's own rates are stored. Each priced row also carries original_currency_code, naming which entry in summary.applied_rates converted it; it is null on core:networth's summed headline. A report that aggregates per currency cannot be priced, and says why in summary.degraded_reason. The accounting convention is negative = expense, positive = income; transfers are exempt.
