...
``ImportService.revert_confirmed`` (see ``moneybin/services/import_service.py``)
consults this allowlist; the revert operation itself lives on the service, not
here.
"""
...
class ImportLogRepo:
    ...
    def get_import_history_page(
        ...
        *,
        ...
        return ImportHistoryPage(
            records=[
                dict(zip(_IMPORT_HISTORY_COLUMNS, row, strict=True))
