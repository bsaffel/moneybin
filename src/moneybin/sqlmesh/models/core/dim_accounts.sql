...
SELECT
  ...
  NOT s.display_name IS NULL AS display_name_is_user_set, /* Whether display_name came from the user-authored app.account_settings arm rather than one of the generated-descriptor fallback arms above. Provenance signal only -- never for display. AccountResolver's weak "name" merge signal reads this to require a user-chosen name, because two unrelated accounts whose *generated* descriptors coincide (e.g. two typeless checking accounts at the same bank both resolving to "checking") is a coincidence of shared attributes already compared separately, not name evidence. */
