...
CREATE TABLE IF NOT EXISTS app.match_decisions (
    ...
    match_type VARCHAR NOT NULL DEFAULT 'dedup' CHECK (match_type IN ('dedup', 'transfer')), -- dedup or transfer (transfer added by matching-transfer-detection.md)
    ...
    match_status VARCHAR NOT NULL CHECK (match_status IN ('pending', 'accepted', 'rejected', 'reversed')), -- pending, accepted, rejected, reversed
