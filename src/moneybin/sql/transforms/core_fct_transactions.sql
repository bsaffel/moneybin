-- Inline transform: core.fct_transactions
-- Extracted from dbt/models/core/fct_transactions.sql
...
CREATE OR REPLACE TABLE core.fct_transactions AS (
    WITH ofx_transactions AS (
        SELECT
            transaction_id,
            account_id,
            CAST(date_posted AS DATE) AS transaction_date,
    ...
    standardized AS (
        SELECT
            ...
            STRFTIME(transaction_date, '%Y-Q%q') AS transaction_year_quarter
