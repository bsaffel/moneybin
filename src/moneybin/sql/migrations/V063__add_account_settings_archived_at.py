"""V063: retire the archive cascade; add app.account_settings.archived_at.
...
DELETION-shaped: undoing a pre-V063 account's first-ever settings write (that
...
def migrate(conn: object) -> None:
    ...
    for (account_id,) in archived_accounts:
        ...
        if row is None:
            no_evidence += 1
            logger.debug(
                f"V063: no archive audit row for account {account_id}; "
        ...
        dated += 1
    logger.debug(
        f"V063: backfilled archived_at for {dated} accounts, {no_evidence} "
