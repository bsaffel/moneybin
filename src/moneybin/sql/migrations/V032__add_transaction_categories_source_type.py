...
- source_type absent (existing DB pre-V032): add with DEFAULT, tighten.
...
def migrate(conn: object) -> None:
    ...
    if "source_type" not in col_map:
        logger.debug(
            "V032: ADD COLUMN app.transaction_categories.source_type "
            "(DEFAULT 'internal')"
        )
        ...
        logger.debug(
            "V032: backfilling nullable app.transaction_categories.source_type"
        )
        conn.execute(  # type: ignore[union-attr]
    ...
    if needs_tighten:
        ...
        logger.debug("V032: ALTER COLUMN source_type SET NOT NULL")
        conn.execute(  # type: ignore[union-attr]
            "ALTER TABLE app.transaction_categories "
