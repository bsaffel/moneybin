        ...
        if rows:
            return InvariantResult(
                ...
                detail=(
                    ...
                    "split. Run `moneybin accounts links run` to raise a "
                    "proposal, `accounts links pending` to review it, and "
                    "`accounts links set <decision_id> --into <account_id>` to "
                    "merge. If the accounts are genuinely distinct, decide the "
                    "proposal standalone and this stays as a warning"
                ),
                affected_ids=[
