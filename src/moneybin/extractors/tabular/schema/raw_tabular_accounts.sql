...
CREATE TABLE IF NOT EXISTS raw.tabular_accounts (
    account_id VARCHAR NOT NULL,                -- Source-native account key, NEVER a canonical MoneyBin account id: the file's own account column (slugified), --account-name (slugified), a PDF document digest, or a content key for an unnamed single-account file. app.account_links maps it to the canonical id; --account-id pins that mapping without changing this value
