...
*display name* (Monarch ``Everyday Spending (...7777)``, ``Checking ····7777``,
``Savings x7777``, ``Card ending in 7777``). Per Decision 8 of
...
4-digit group (``WF CHECKING 3030``) is accepted as a last4 — a 4-digit year
...
_LAST4_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"\bx(\d{4})\b", re.IGNORECASE),  # x7777
    re.compile(
        r"(?<!\d)(\d{4})\s*$"
