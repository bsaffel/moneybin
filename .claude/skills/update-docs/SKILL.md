...
- `docs/reference/` — lookup material and internal-mechanics deep-dives
  ...
  - `docs/reference/cli/`, `docs/reference/mcp-tools.md`, and
    `docs/reference/configuration.md` are **generated** by `make generate-docs`
    from the command tree, the MCP server's tool list, and `MoneyBinSettings`.
    Never hand-edit them: fix the help text, tool description, or field
    description at its source and regenerate. They open with a
    `<!-- Generated by … -->` marker and carry no "Last reviewed" stamp.
...
entirely, add it on first edit using today's date. Generated pages are the
one exception: their generator marker is the review, and they never carry
a stamp.
