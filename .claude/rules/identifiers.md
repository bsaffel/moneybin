...
**None of the three holds one kind of value.** `source_account_key` is always a
source-native key, but never uniformly the institution's: `<ACCTID>` on OFX, the
provider's own account id on Plaid, the sheet's own key on Sheets, and
MoneyBin-synthesized only where the channel supplies none; `account_id`
surfaces a source-native key whenever the resolver never linked the account;
`proposed_account_id` on the mint path is a preview that is never written; and
`@N` indexes the file's full detected list, not the proposals you can see —
renumbering the surfaced list to start at `@0` binds the wrong account. The
per-channel table and the citations behind each case:
[`.claude/references/account-identifier-provenance.md`](../references/account-identifier-provenance.md).
Read it before touching an import channel, the account resolver, or a surface
that prints an account reference — `proposal_ref` is the referent to put in
front of a user or an agent, and that is why it exists.
...
**Changing an id leaves the old one behind — retire it in the same change.** Suffixing every colliding member means a FITID that arrives *alone* in one export and *collides* in another is written bare the first time and suffixed the second. The bare row's primary key no longer matches, so the later import cannot overwrite it, and staging partitions on the id — both reach `core.fct_transactions` and one real transaction is counted twice. It reads as a double-count, never as a duplicate id, so no uniqueness guard fires. `prep.stg_ofx__transactions` suppresses the superseded row by pairing it with the suffixed twin that carries identical content on every field the suffix hashes.

Two rules generalize from that:
