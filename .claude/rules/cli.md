...
**Enforcement:** `tests/moneybin/test_architecture/test_adapter_layering.py` fails CI when CLI commands (or MCP tools) import write-callable symbols from `moneybin.extractors` or `moneybin.matching` without an allowlist entry. The fix is almost always a new service method; allowlist entries are reserved for pure constants, pure read helpers, DI targets, and type/format descriptors, each with a `# why` comment.
