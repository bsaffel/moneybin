...
Each of these has already cost a debugging detour. The first nine are failures that **look** like they need a bypass and do not.
...
| A script's own `security find-generic-password` **read** for a non-`moneybin` Keychain item (e.g. PAO's `linear_provider.py` reading `pao-linear-api`) → "credential is unavailable in Keychain" even though the item exists | The sandbox's filesystem read allowlist never includes `~/Library/Keychains` — the login keychain isn't visible in-sandbox at all, so `security` can't find an item that's really there. | Don't widen the sandbox's Keychain read allowlist — that exposes every credential in the login keychain, not just the one needed. Instead resolve the secret once, outside the sandbox and outside Claude Code, in the shell that launches `claude` — the same mechanism `GH_TOKEN` already uses (confirmed: `GH_TOKEN` is set in the sandboxed Bash env, but `.claude/settings.local.json`'s own `env` block is empty, so it's injected by a launch-time shell wrapper, not by Claude Code): `claude() { GH_TOKEN="$(gh auth token 2>/dev/null)" LINEAR_API_TOKEN="$(security find-generic-password -s pao-linear-api -a default -w 2>/dev/null)" command claude "$@"; }`. Give the script an env-var-first check (`os.environ.get("LINEAR_API_TOKEN", "").strip()` before the `security` subprocess call, keeping the subprocess call as a fallback) so it runs fully sandboxed once the wrapper exports it. |
| A Python `urllib` request from a script with its own `build_opener(ProxyHandler({}), ...)` → `URLError: [Errno 8] nodename nor servname provided, or not known` even after the host is added to `sandbox.network.allowedDomains` | The sandbox's network egress *is* a real filtering proxy, exposed via standard `http_proxy`/`https_proxy` env vars — the same mechanism `gh`/`pip`/`uv` already rely on. `ProxyHandler({})` explicitly tells `urllib` to ignore those and connect directly, which the sandbox doesn't route; the allowlist entry is correct but irrelevant to a direct connection. | Don't pass an empty `ProxyHandler`; build the opener without one (e.g. `build_opener(_RejectRedirects)`) so `urllib` picks up the env-set proxy by default, matching `gh`'s behavior — verified fix in PAO's `linear_provider.py`. This is a real security tradeoff, not just a technical toggle: honoring `https_proxy` means trusting whatever that variable is set to in *any* environment the script runs in, not just this sandbox. Weigh it explicitly for a credential-bearing request rather than defaulting to it silently. Also check `sandbox.network.allowedDomains` for stray whitespace in the JSON string (`"api.linear.app "` silently failed to match) — the sandbox only picks up allowlist edits after a `/sandbox` toggle or a fresh session, not on save. |
