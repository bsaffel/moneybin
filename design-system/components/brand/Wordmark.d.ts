 ...
 * optical baseline nudge (Rule 1) and brand-gold on "Bin" (Rule 2), both baked in.
 */
export interface WordmarkProps {
  ...
  /**
   * Color of the "Bin" letters. 'gold' (default) uses --brand-gold — bright gilt
   * on dark, deepening to brass on light; use 'mono' whenever another gold element
   * shares the same bar or header, so the gold stays unique to a single lock-up.
   */
  bin?: 'gold' | 'mono';
  /**
