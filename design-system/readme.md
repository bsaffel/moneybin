...
## Visual foundations
- **Color:** warm ink/paper near-neutrals; ONE metal accent in three tiers — brass speaks (provenance text), gilt fills (bars, dots, buttons, the wordmark), verdigris responds (selection, filters, links); never blue. Dark theme leads (audience lives in dark editors); light is first-class (marketing/print lead). Income/expense pair survives color-blindness; sign is ALWAYS redundant (+/−, ▲/▼). Accounting convention: negative = expense.
...
- `patterns.md` — app-pattern grammar (FilterBar, SegmentedControl, PageHeader, NavRail, Table, VaultStatusBar) + the canonical synthetic dataset
