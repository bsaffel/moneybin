...
- **CLI terminals:** use semantic hierarchy, context, action, and state styles from `moneybin.cli.render.Style`, resolved by `moneybin.cli.terminal.TerminalPolicy`. Respect the terminal palette, `NO_COLOR`, ASCII fallback symbols, narrow widths, and reduced motion. Terminal output does not assume a web font, a background colour, or emoji.
