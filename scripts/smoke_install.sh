...
echo "==> installed wheel categorization merchant learning"
"$VENV/bin/python" - "$WORK_DIR/merchant-learning-proposal.json" <<'PY'
import json
import sys
...
with get_database(read_only=False) as db:
    ...
    transaction_id = result.results[0].transaction_id
...
with get_database(read_only=True) as db:
    merchants = db.execute(
        ...
        "WHERE canonical_name = 'Smoke Merchant'"
    ).fetchall()
assert merchants == [("Smoke Merchant",)]
PY
